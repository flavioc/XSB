...
:- bootstrap_package('bounds', bounds).
...
% TLS: we should probably implement call/n.
sum1([],Sum,Op,Value) :-
	functor(Term,Op,2),
	arg(1,Term,Sum),
	arg(2,Term,Value),
...
*/
/* Change because XSB doesnt have user-defined functions */
max_divide(L1,U1,L2,U2,Max) :-
	( L2 =< 0 , U2 >= 0 ->
		max_inf(Max)
	;
	  ...
	  div(U1,L2,UL12),
	  div(U1,U2,UU12),
	  Max is max(max(LL12,LU12),max(UL12,UU12))
	).	
min_divide(L1,U1,L2,U2,Min) :-
	( L2 =< 0 , U2 >= 0 ->
		min_inf(Min)
	;	
	  ...
	  div(U1,L2,UL12),
	  div(U1,U2,UU12),
	  Min is min(min(LL12,LU12),min(UL12,UU12))
	).
...
mydiv(X,Y,Z,New) :-
	...
	; nonvar(Z) ->
		...
		(YL >= 0, XL >= 0 ->
			NXL is max(YL*Z,XL),
		        % TLS: had to reorder for parser
			NXU is min((-1 + YU*(Z + 1) ),XU)
		; % TODO: cover more cases
			NXL = XL,
			NXU = XU
		),
...
:- install_verify_attribute_handler(bounds,Attr,Other,bounds_attr_unify_hook(Attr,Other)).

bounds_attr_unify_hook(bounds(L,U,Exp),Other) :-
...
% TLS: probably using unifiable incorrectly.
