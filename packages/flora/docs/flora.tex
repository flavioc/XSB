...
\title{\FLORA's Secrets: An Object-Oriented Logic Programming and Deductive Database System}
...
\index{FLIP}
The current version of \FLORA is implemented by Guizhen Yang under the
supervision of Michael Kifer, and its  origins trace back to the \FLIP compiler
developed by Bertram Lud\"aescher. The basic architectures of the two compilers
are similar. However, unlike \FLIP, \FLORA is a complete application development
...
$\texttt{'JOHN SMITH'},\texttt{'default.flr'}$.
...
the aggregation variable into a list. The operator {\it collectset} works similarly
to {\it collectbag}, except that all the duplicates are removed from the
result list.

In general, aggregates can appear wherever a number or a list is allowed. Therefore,
...
\item \verb|a[m ->> c]| was derived based on the belief that attribute {\tt
  ...
  the original inheritance was based, so we must undo the opeation and the
  ensuing rule application.
\item We did derive \verb|a[m ->> d]| as a result of inheritance, but
  that's OK --- we should not really be looking back.
\end{enumerate}
%%
...
attributes. Clearly, there is no reason to disalow inheritable attributes
...
signatures goof for then? One answer is that future versions of \FLORA
...
A meta variable is specified by a normal variable immediately following a ``@''
sign, {\it e.g.}, @Method, @\_var, @\_. Note that @\_ represents a \emph{don't care}
meta variable. The ``@'' sign is always considered as part of a meta variable's name.
Thus, @M and M represent two different variables.

Currently, a meta variable can only appear where a method together with its arguments
is allowed. For instance, $\mathtt{john[@M{\fd}Salary], o_1.@M1.o_2[@M2{\mvd}r]}$.

Note that a meta (method) variable does not represents an \fl object, but a method
invocation expression instead, {\it i.e.}, a method together with its arguments.
The =.. operator (like that of XSB) is used to obtain a method and its arguments from
a method invocation expression (that is bound to a meta variable), or on the other hand,
to build a method invocation expression out of a method and its arguments. For example,
\begin{verbatim}
FLORA> ?- @M =.. [m,a1,a2].
...
The left hand side of =.. can be either a normal Prolog term or a meta variable. In the
former case, =.. just works exactly the same as that of XSB, whereas in the latter case,
the right hand side of =.. would try to unify with a list of \fl objects containing the
method object as the first element followed by its argument objects.

Consider the example in Figure~\ref{fig:fig-metavar}:
%%
\begin{figure}[htb]
\begin{center}
\begin{tabular}{l}
...
The rule says that applying to $o_2$ those of $o_1$'s methods that take two arguments 
yields the same result as applying to $o_1$. To see the effects, from \FLORA
shell's command prompt, enter:
\begin{verbatim}
...
work at the present time. The reason is that \FLORA compiler always
treats \fl molecules as oid's, if they appear as predicate arguments.
However, in {\tt findall}, we want the molecules in the second argument to
