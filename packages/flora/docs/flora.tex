...
\title{\FLORA's Secrets: Object-Oriented Logic Programming}
...
\index{FLIP}
The current version of \FLORA was implemented by Guizhen Yang under the
supervision of Michael Kifer, but its  origins trace back to the \FLIP compiler
...
``no'' answer means that the corresponding attribute does NOT violates the
typing rules.
...
that originate from $Q_2$ are followed.
