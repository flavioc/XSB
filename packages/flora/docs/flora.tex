...
\begin{qrules} ?- Z = min\{S; john[salary@(Year){\fd}S]\}. \\ ?- Z =
count\{Year; john.salary@(Year) $<$ max\{S; john[salary@(Y){\fd}S],
Y$<$Year\}\}. \\ ?- avg\{S[Who]; Who{\isa}employee[salary@(Year){\fd}S]\}
$>$ 20000.  \end{qrules}
...
``@'' sign, {\it e.g.}, @Method, @\_var, @\_. Note that @\_ represents a
\emph{don't care} meta variable. The ``@'' sign is always considered as
part of a meta variable's name.  Thus, @M and M represent two different
variables.
...
Note that a meta (method) variable does not represent an \fl object, but a
method invocation expression instead, {\it i.e.}, a method together with
its arguments.  The =.. operator (like that of XSB) is used to obtain a
method and its arguments from a method invocation expression (that is bound
to a meta variable), or on the other hand, to build a method invocation
expression out of a method and its arguments. For example,
\begin{verbatim}
FLORA> ?- @M =.. [m,a1,a2].
...
The left hand side of =.. can be either a normal Prolog term or a meta
variable. In the former case, =.. just works exactly the same as that of
XSB, whereas in the latter case, the right hand side of =.. would try to
unify with a list of \fl objects containing the method object as the first
element followed by its argument objects.
