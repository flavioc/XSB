...
\end{quote}
%%
%% UNIMPLEMENTED -- meta programming
...
%% UNIMPLEMENTED --  :-table implies :-firstorder
...
%%UNIMPLEMENTED -- make base part dynamic
Note that the base part of a predicate or an object contains only the facts
that were \emph{explicitly inserted} into the database. If you just put the
facts such as 
%%
{\tt
\begin{quote}
...
%%
\noindent
into the database, they will be located in the derived part and cannot be
deleted. (This might change in a future release).

%%UNIMPLEMENTED
Also, in the current release, the base part of first-order predicates
cannot be queried in the \FLORA shell (but it can be queried in the program
file).  Likewise, if the {\tt firstorderall} directive is in effect, the
base part of the first-order predicates is inaccessible to querying.  These
...
work (will leave the database unchanged) for obvious reasons. In a future
release, we will provide {\tt while}-loops that will work correctly with
such updates.

%% UNIMPLEMENTED -- backtrackable updates and the fail-loop
...
%% UNIMPLEMENTED -- checking dependency of tabled preds on update
%% primitives
...
begin with {\tt :-} in contrast to {\tt ?-} for queries.  The following is
a list of all the compiler directives supported by \FLORA:
%%
\begin{itemize}
...
\item {\tt table {\it functor/arity}, ..., {\it functor/arity}}
  ...
  %% UNIMPLEMENTED -- table implies firstorder
  (Currently this also requires an explicit {\tt firstorder} directive, but
...
This library allows one to pretty print the information about an object or
about all objects in a given class. This information can be saved in a file
or printed on the screen. The library should be first loaded into some
module, say
%%
\begin{quote}
 {\tt
...
\end{quote}
%%
To pretty print information about an object, the following calls can be used.
The first argument is the module name of the calling object.
%%
\begin{itemize}
...
\begin{quote}
 {\tt
   ?- flThisModule(M), john[\#pp\_self(M)].
   }
\end{quote}
%%
...
%% UNIMPLEMENTED
This has not been implemented yet, but this is how the initial version will
look like.
...
the program into the default module named {\tt main}. If {\tt
  myprog.flr} contains \fl facts, all these facts will be compiled
separately into the XSB file {\tt myprog.fdb} that is dynamically
loaded at runtime.\footnote{
  %%
  %% UNIMPLEMENTED
  Currently these facts are compiled, but will be loaded dynamically in the
  future.
  %%
  }
%%
Next, the file {\tt myprog.P} is generated --- take a look at ``{\tt
...
{\tt genincludes/flrpatch.fli}.  The patch rules for the first-order
