...
\item {\tt flReset(\{arguments|op\})}:
    Clear all dynamic {\tt arguments/op}  definitions in the
    \FLORA shell.
\item {\tt flAll}:
...
\subsection{Two Operators for Negation}
\index{tnot}
\index{\NAF}
%%
...
called {\rm trie}, and are retrieved using  ``patch rules'' such as
%%
\begin{verbatim}
...
d_ifd <- ifd
          fd <-      inferred_fd
          fd <- tnot inferred_fd      <- derived_fd <- base_fd
     ...
     base_fd <- storage_find_fact(...trie_name..., fd(...))
\end{verbatim}
%%
...
{\tt .xsb/flora/}.  As mentioned earlier, \FLORA uses different names for
...
which {\tt flrpatch.flh} is generated during the installation).
