...
produces {\tt file.\ofile}) and execution, which, however, requires \FLORA
runtime support.
...
scheduling). This problem will be fixed in a future release of XSB.
Fortunately, beginning with XSB version 2.5 this is the default
configuration.  If (for some reason) you need to configure 
XSB differently, you can do so using these two commands:
%%
{\tt
\begin{quote}
 configure --enable-batched-scheduling --config-tag=\\
 makexsb
\end{quote}
}
%%
If you are using Unix --- you are a lucky person, because \FLORA
is built together with XSB, so you do not need to do anything special to
have it working. If, for some reason it fails to build, follow
these steps:
%%
\begin{verbatim}
...
\end{verbatim}
%%
At present \FLORA does not work under Cygwin because XSB's C interface is
not working well with Cygwin.
...
In the following command list, the suffixes {\tt .flr} {\tt .P}, {\tt .\ofile}
...
XSB program, and {\tt .\ofile} means that it is a bytecode file, which can be
...
tries the suffix {\tt .P}. Otherwise, it tries {\tt .\ofile} in the hope that an
...
might occur in the head of a rule that has an update in its body, or it may
be transitively dependent on such an update. In this case, the update will
be executed the first time the predicate is evaluated. Subsequent calls
will return the predicate truth value from the tables, without invoking the
predicate definition.  Moreover, if the update statement is non-logical
({\it i.e.}, non-backtrackable), then it is hard to predict how many times
it will be executed (due to backtracking) before it will start being
ignored due to tabling.
...
\paragraph{Note:}
At present, XSB has serious problems with its implementation of
{\tt assert/retract} predicates, which affects {\tt while-do} and {\tt
  do-until}. Therefore, it is recommended to avoid these control
constructs.

\subsection{Pitfalls in Using Loops}

Observe that typically the {\tt while/until} loops are used in situation
when the condition in the loop is being updated inside the loop body.
Therefore, the condition must \emph{not} contain tables predicates.
If such predicates are involved in the loop condition, the loop is likely
to execute infinitely many times, and can even be infinite.
...
\section{Bugs in XSB and \FLORA: How to Report}
...
An incorrect result during the execution can be equally blamed on XSB or
...
Bugs that are the fault of XSB are particularly hard to fix, because \FLORA
programs are translated into mangled, unreadable to humans Prolog code.
To make things worse, this code might contain calls to \FLORA system
libraries.
...
\paragraph{Reporting \FLORA-related XSB bugs.}
...
\begin{enumerate}
  ...
  \item If the program still does not run because of the hard-to-get-rid-of
    ...
    \end{quote}
    %%
    in the XSB shell. If the program runs after this (and reproduces the
...
passed to the XSB compiler, yielding Prolog byte code {\tt myprog.\ofile}, which
