...
\begin{enumerate}
  ...
  \item  Eliminate all the calls to the system services (that use the
    {\tt @flora(lib)} syntax).
...
\begin{enumerate}
  \item Reduce the size of the program as much as possible by deleting 
    ...
    just one file.
  \item Remove all the calls to service libraries (that use {\tt
      @flora(lib)} syntax), unless such a call that is the essence of the
    bug.
\end{enumerate}
%%
