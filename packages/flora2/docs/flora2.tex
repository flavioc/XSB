...
norm. To get back to the Prolog command loop, type {\tt Control-D} (in
Unix), Control-Z (Windows), or 
%%
\begin{quote}
  \tt
| ?- flEnd.  
...
\item
  \index{arguments!compiler directive}
  %%
  {\tt arguments Functor(\{oid$|$meta\}, ...)}:
...
\item
  \index{{\tt flReset}}
  %%
  {\tt flReset(\{arguments|op\})}:
    Clear all dynamic {\tt arguments}  definitions or operator definitions
    in the \FLORA shell.
\item {\tt flAll}:
...
Prolog to recompute the results. This is accomplished by issuing the
predicate {\tt abolish\_all\_tables/0} described in the XSB manual. We
discuss updates in Section~\ref{sec-updates}.

Note: {\tt abolish\_all\_tables/0} \emph{cannot} be used in
%%
\begin{enumerate}
  \item  the body of a rule that defines an object attribute or method
  \item  in the body of a rule that defines a tabled HiLog predicate
\end{enumerate}
%%
This is because internally the above entities are represented using tables
predicates. Execution of {\tt abolish\_all\_tables/0} in the body of such a
rule would destroy the table for the predicate being computed by the rule.

More generally, no tabled predicate or object molecule of the above sort
can depend via rules --- directly or indirectly --- on {\tt abolish\_all\_tables/0}.
However, it is safe to use this predicate in the body of a rule that
defines a procedural method (defined next).
...
one used in Prolog. The problem is that {\tt foo(a,b,c)} is a HiLog term that
...
\index{catch/3}
%%
The undefinedness exception in \FLORA can be caught using the {\tt
  catch/3} builtin provided by the underlying Prolog engine. For instance, suppose
...
%%
\begin{verbatim}
  ..., catch(FOO, flora_undefined_exception(Call), handler(Call)), ...
\end{verbatim}
%%
(There is no need to indicate {\tt @prolog()} for {\tt catch/3} because
{\tt catch/3} is also considered a \FLORA builtin.)
Here {\tt handler/1} is a user-defined predicate (can be a molecule as well,
...
ignored and the call to $FOO$ will succeed (but the error message would be
printed nonetheless).
...
The third way is to ignore the exception using the {\tt catch/3} builtin:
%%
\begin{verbatim}
 ?- catch(X:a, flora_undefined_exception(_), true).
\end{verbatim}
%%
