...
By default, all \FLORA programs are loaded into the module {\tt main}.
...
\item {\tt flOp(Precedence,Associativity,Operator)}:
    Define an operator in shell mode.
\item {\tt flReset(flProlog$|$flPredicate$|$flSign$|$op)}:
...
\item \emph{Rules} are, as usual, the constructs of the form $head :-
  ...
  F-molecules or negated F-molecules. (Negation is specified using {\tt
    not} or {\tt tnot} --- the difference will be explained later.)
...
\begin{qrules}
{\tt :- flOp(\emph{Precedence},\emph{Type},\emph{Name}).}
\end{qrules}
%%
For instance, 
%%
...
through the prefix operators ``{\verb|\+|}'', ``{\tt not}'', and ``{\tt
  tnot}''.\footnote{
  %%
...
\item Module specification is distributive over logical connectives,
  ...
  ``\verb|;|'', and the negation operators, ``\verb|\+| and
...
\begin{quote}
  \tt
  ?- {\it predicate}@prologlib({\it module})  
\end{quote}
%%
...
\begin{quote}
 \tt
 ?- member(abc,[cde,abc,pqr])@prologlib(basics).
\end{quote}
%%
...
Since XSB does not understand object-oriented syntax, it can call only the
predicates defined in \FLORA programs. These predicates can be HiLog or
\FLORA predicates (See Section~\ref{sec:hilog}). To call predicates
...
\begin{quote}
  \tt
   ?- flImport \{flHilog|flPredicate\} {\it flora-predicate/arity} as {\it
     xsb-name}(\_,\_,...,\_)\\
...
In both cases, the option {\tt flHilog} is used to import a HiLog predicate
and {\tt flPredicate} is used to import \FLORA predicates. The imported
...
as a predicate skeleton with the same number of arguments as in the \FLORA
predicate. For instance, {\tt foo(\_,\_,\_)} will do, but {\tt foo/3} will
not.
...
list {\tt L}. This feat is accomplished using the {\tt flSign} compiler
instruction.
...
directive {\tt flSign}.  For instance, the following directive will make
{\tt findall/3} function as intended:
%%
\begin{qrules}
:- flSign findall(oid,bform,oid).
\end{qrules}
%%
...
\end{quote}
where {\tt functor} stands for the functor of a Prolog term and
{\tt arity} stands for its arity.

Note that the above {\tt flProlog} compiler directive does not affect
...
\end{quote}
then a term like {\tt member(foo(a),X)} in this program will be translated
into {\tt member(flapply(foo,a),X)}.
...
\subsection{\FLORA Predicates and Prolog Predicates}
...
integration with the host language (XSB) and HiLog. As a result, \FLORA has
not one type of predicates but three!
...
To avoid these semantic problems, \FLORA supports the {\tt flPredicate}
declaration, which defines certain symbols as \FLORA predicates. For
instance,
%%
...
\FLORA predicates are module-aware. This means that the predicates defined
in different modules are treated as unrelated entities.

There is one other reason for using \FLORA predicates over HiLog
predicates, which has to do with tabling. This issue is discussed in
Section~\ref{sec-tabling-flora}.


The third type of predicates, called \emph{Prolog predicates}, is the
vehicle that allows \FLORA to call XSB predicates with ease. 
...
%%
\index{flEquality}
\index{compiler directive!flEquality}
%%
{\tt
\begin{quote}
:- flEquality none.\\
...
:- flTable defined/2, overwritten/3, conflict/3.
...
:- flTable definedSet/2, overwrittenSet/3, conflictSet/3.
...
\index{flTable}
\index{compiler directive!{\tt flTable}}
%
...
\FLORA implementation). To table a predicate, it must be
declared 
as a \FLORA predicate using the
\mbox{\tt flPredicate functor/arity} compiler directive (refer to
Section~\ref{sec:hilog} and Section~\ref{sec:flora-modules-predicates}
...
\end{quote}
%%
renders {\tt tc/2}  as a tables \FLORA predicate.

\index{flHilogTable}
\index{compiler directive!{\tt flHilogTable}}
%%
...
\index{compiler directive!{\tt flPredicate}}
\index{compiler directive!{\tt flPredicateAll}}
\index{compiler directive!{\tt flAbolish}}
\index{compiler directive!{\tt flEquality}}
\index{compiler directive!{\tt flSign}}
\index{compiler directive!{\tt flOp}}
%
...
by \FLORA:
\begin{itemize}
\item {\tt flAbolish} {\em functor/arity}, ..., {\em functor/arity}
  \\
  Abolishes the listed \FLORA predicates in the current program's module,
  ...
  which the program that contains this particular {\tt flAbolish} directive
  is loaded.
\item {\tt flAbolish} {\em functor/arity}, ..., {\em functor/arity} {\tt
    in} \emph{module}\\
  Same as above, except the predicates are abolished in the specified
...
\item {\tt flEquality none|basic|flogic} {\tt in}  \emph{module}
  \\
  Same as above, except that equality maintenance is set for the specified
  module.
\item {\tt flPredicate} {\em functor/arity}, ..., {\em functor/arity}
  \\
  Declare the listed predicates as \FLORA predicates (as opposed to HiLog
  predicates).
\item {\tt flPredicateAll}
  Makes all predicates into \FLORA predicates. (The default for predicates
  is HiLog.)
\item {\tt flProlog} {\em functor/arity}, ..., {\em functor/arity}
  \\
  Declares the listed predicates to be Prolog predicates (rather than HiLog
  predicates).
\item {\tt flPrologAll}
  \\
  Makes all predicates into Prolog predicates.
\item {\tt flSign} \emph{functor}(\emph{type}, ..., \emph{type}), where
...
\item {\tt import {\it functor/arity}, ..., {\it functor/arity} from
    {\it module}}
  \\
  Imports the given Prolog predicate from the specified XSB module.
\item {\tt index {\it functor/arity-argNumber}}
  \\
  Specifies that the given predicate must be indexed on the specified
  argument.
\item {\tt flOp({\it precedence},{\it type},{\it operator})}
  \\
  ...
  {\tt fx}, {\tt xf}, {\tt xfy}, etc.
\item {\tt flOp({\it precedence},{\it type},[{\it operator}, ..., {\it operator}])}
  \\
