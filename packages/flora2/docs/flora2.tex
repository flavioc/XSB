...
\item {\tt reset(prolog$|$nohilog$|$sign$|$op)}:
    clear all dynamic prolog/nohilog/sign/op definitions in shell mode
\item {\tt all}:
...
equality maintenance. Since {\tt a:=:a} only trivially identifies
the oid {\tt a} with itself, its net effect is just to force the
...
pass its object value to the predicate or function. If the truth
value of a path expression is intended, the compiler directive
{\sf sign} can be used to signal how the compiler should flatten
the arguments. For instance, the following compiler directive is
needed to make {\sf findall/3} function correctly in \FLORA:
\begin{qrules}
...
\subsection{HiLog and Related Issues}
...
In general, the presence of a pair of parenthese in a term would imply
...
{\tt nohilog functor/arity}, can be used to turn off the HiLog
...
a predicate call or as an argument. However, \mbox{\tt nohilog functor/arity}
...
the {\tt nohilog} directive for the same functor and arity pair. 

Shown below is an example to illustrate the different effects of the
{\tt prolog} and {\tt nohilog} compiler directives:
\begin{quote}
\begin{verbatim}
:- prolog foo/1.
:- prolog goo/1.
...
\begin{quote}
\verb|?- member(a,[a]) @ xsb basics, member(X,[a]).|
\end{quote}
...
************* should add the flora tabling/non-tabling directives ***
...
\FLORA-specific in this? It turns out yes, and that the problem is not
...
effects are subject to the same problem as described above.

 To solve this
program, \FLORA introduces new syntax to escape those methods with side
effects, which are translated into non-tables predicates when the program
is compiled. Refer to Section~\ref{sec:} for information
...
\subsection{Non-backtrackable Updates}

\subsection{Backtrackable Updates}

\subsection{Asserting, Retracting, and Tabling}
...
************* other problems: tabled should not depend on transactions *****

************* tabled queries that depend on dynamic part of the database
              should use wrappers that abort tables on backtracking. ******
...
***** Explain that if negation is applied to the molecule then the molecule
      should not contain transactions.


\section{\FLORA Compiler and Loader} \label{sec-comp-directives}
