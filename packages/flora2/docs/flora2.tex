...
\FLORA shell includes many commands beyond simple loading of a program.
These commands are listed below. However, at this point some of these
commands might seem a bit cryptic, so it is a good idea to come back here
after you become more familiar with the various concepts underlying the
system.
...
\item \emph{Rules} are, as usual, the constructs of the form $head :-
  ...
  {\tt
    \emph{head}~:-~john[age{\fd}31],~(john[children\mvd\{bob,mary\}]~;~
  ...
  \\
  {\tt
    \emph{head}~:-~john[age{\fd}31],~john[children\mvd john].
    }
  \end{quote}
  %%
  ...
  \begin{quote}
 {\tt
   \emph{head}~:-~john[age{\fd}31,~(children\mvd\{bob,mary\}~;~children\mvd john)].
   }
  \end{quote}
  %%
...
object $R_0$ for which $\tt O[M\fd R_0]$ holds; (8) is a \emph{multi-valued}
path expression; it refers to each $R_i$ for which $\tt O[M\mvd\{R_i\}]$ holds.
The symbols $\tt O$ and $M$ stand for an id-term or path a expression.
Moreover, $M$ can be a method that takes arguments, i.e.,
$\tt O..M(P_1,\dots,P_k)$ is a valid path expression.
...
\subsection{Truth Value vs.\ Object Value and the flSign Compiler Directive}\label{sec-references}
...
%
The following aggregate operators are supported in \FLORA: {\tt flMin}, {\tt flMax},
{\tt flCount}, {\tt flSum}, {\tt flAverage}, {\tt flCollectSet} and {\tt flCollectBag}.

The operators {\tt flMin} and {\tt flMax} can apply to any list of
terms. The order among terms is defined by the XSB operator {\tt @=<}.  In
contrast, the operators {\tt flSum} and {\tt flAverage} can take numbers only. If
the aggregate variable is instantiated to something other than a
number, {\tt flSum} and {\tt flAverage} will discard it and generate a runtime
warning message.

For each group, the operator {\tt flCollectBag} collects all the bindings of
the aggregation variable into a list. The operator {\tt flCollectSet} works
similarly to {\tt flCollectBag}, except that all the duplicates are removed
...
of the {\tt flCollectSet} aggregate.
%%
\begin{quote}
\begin{verbatim}
...
a function symbol is allowed, e.g., {\tt group(X)(Y,Z)}, where the
functor is the complex term {\tt group(X)}. In \FLORA, any id-term,
...
\subsection{Referencing Literals in Other Modules}
...
\subsection{The Current Module Name}
...
\begin{itemize}
\item {\tt auto\_table}
\item {\tt dynamic} {\em functor/arity}, ..., {\em functor/arity}\\
  Declares the listed Prolog predicates as dynamic ({\it i.e.}, updatable).
  These predicates must be earlier declared as {\tt flProlog}.
  This directive might be deprecated in future releases, because Prolog
  predicates are intended to be \emph{defined} on the XSB side and only
  \emph{used} on the \FLORA side.
\item {\tt export} {\em functor/arity}, ..., {\em functor/arity}\\
  This directive \emph{should not be used} in \FLORA programs. Its only
  purpose is to permit running XSB programs that have the {\tt export}
  directive under the \FLORA system. Even this purpose is highly
  questionable and this directive might be deprecated in the future
  releases.
