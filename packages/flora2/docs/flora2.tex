...
\item {\tt flEquality(\{none$|$basic$|$flogic\},Module)} :
    load the standard/basic equality/F-logic equality trailer into the module `Module'
\item {\tt flEquality \{none$|$basic$|$flogic\}}:
    load the standard/basic equality/F-logic equality trailer into the default module `main'
...
\item {\tt flSign Functor(\{oid$|$form\}[,\{oid$|$form\}]*)}:
...
\item {\tt reset(flProlog$|$flPredicate$|$flSign$|$op)}:
    clear all dynamic flProlog/flPredicate/flSign/op definitions in shell mode
\item {\tt all}:
