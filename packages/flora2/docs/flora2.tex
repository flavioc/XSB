...
predicate {\tt flModule/1} for that. For instance, the first query, below,
succeeds if the module {\tt foo} is loaded. The second query succeeds and
binds {\tt L} to the list of all user modules that are loaded at the
present time.
%%
{\tt
\begin{quote}
  ?- flModule(foo).\\
  ?- findall(X,flModule(X),L).
\end{quote}
}
%%
...
program file defining it into a given \FLORA user module. The second syntax
is used when the flora program is already loaded into a module and we only
need to import the corresponding predicate.
...
%%
  ...
  different operation depending on who is calling it, because {\tt
    something} can be defined differently for different callers. This is,
  for instance, how the pretty printing system module of \FLORA works. A
  pretty-printing method is called on an object in some user module, and to
  do its job the pretty-printing method needs to query the object \emph{in
    the context of the calling module} to find the methods the object has.
...
shell. Furthermore, \FLORA allows one user module to set the level of equality
maintenance in another user module:
%%
\begin{quote}
...
\end{quote}
%%
This will insert {\tt foo(a,b,c)} into the catch-all XSB module called
...
the {\tt @flora(lib-name)} system module and (unlike user modules)
