...
from \FLORA's point of view, XSB modules are always available and do not
need to be loaded explicitly. Because the association between Prolog
programs and modules is fixed, these modules are called \emph{static prolog
  modules}.  The syntax to call XSB predicates is
%%
...
\subsection{Flora Static Modules}
...
reason, these modules are called \emph{static \FLORA modules}.  For
instance, to prettyprint all the attributes and methods of an object, the
following method, defined in the static {\tt pp} module, can be used:
%%
\begin{quote}
...
more details on the existing \FLORA static modules, see
...
\section{\FLORA Static Modules}\label{sec-service-libs}
...
\emph{static \FLORA modules}.  We describe the functionality of these
modules below.
...
static module {\tt pp} and is accessible using the {\tt @flora(pp)}
syntax.
...
This library is preloaded into the static module {\tt io} and
...
\section{A Note on the Programming Style}
...
\begin{enumerate}
  ...
  \item  Eliminate all the calls to the static modules that use the
    {\tt @flora(lib)} syntax. (Prolog static modules that are accessible
    through the {\tt @prolog(modname)} syntax are OK, but the more you can
    reduce the size of the program the better.)
  \item  If the program has several modules, try to put them into one file
    and use just one module.
...
\begin{enumerate}
  \item Reduce the size of the program as much as possible by deleting 
    ...
    just one file.
  \item Remove all the calls to static modules (that use {\tt
      @flora(mod)} syntax), unless such a call that is the essence of the
    bug.
\end{enumerate}
%%
