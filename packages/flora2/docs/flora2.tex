...
\item {\tt reset(prolog$|$flpred$|$sign$|$op)}:
    clear all dynamic prolog/flpred/sign/op definitions in shell mode
\item {\tt all}:
...
\index{compiler directive!{\tt flpred}}
%
...
{\tt flpred functor/arity}, can be used to turn off the HiLog
...
a predicate call or as an argument. However, \mbox{\tt flpred functor/arity}
...
the {\tt flpred} directive for the same functor and arity pair. 

Shown below is an example to illustrate the different effects of the
{\tt prolog} and {\tt flpred} compiler directives:
\begin{quote}
\begin{verbatim}
:- prolog foo/1.
:- prolog goo/1.
...
\index{compiler directive!{\tt flpredall}}
%
...
\begin{quote}
\verb|?- member(a,[a]) @ xlib basics, member(X,[a]).|
\end{quote}
...
\mbox{\tt flpred functor/arity} compiler directive (refer to
...
\item \verb!flpred functor/arity, ..., functor/arity!
\item \verb!flpredall!
