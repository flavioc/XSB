/******************************************************************************
 *                              model.c
 * This file contains functions which implement the finite state engine which 
 * is used to validate the xml document 
 *
 ****************************************************************************/
...
static void
mylink(dtd_state *from, dtd_state *to, dtd_element *e)
...
{ if ( m->type == MT_PCDATA )
  { mylink(from, from, CDATA_ELEMENT);
    mylink(from, to, NULL);
    return;
  }
...
{ 
  ...
  { dtd_edef *def = e->structure;
	...
	mylink(def->initial_state, def->initial_state, CDATA_ELEMENT);
	mylink(def->initial_state, def->final_state, NULL);
      } else
	return NULL;
    }
