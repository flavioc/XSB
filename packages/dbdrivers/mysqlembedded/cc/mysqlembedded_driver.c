...
/*
** This is the driver for connecting to a MySQL Embedded Server database.
...
#include "mysqlembedded_driver_defs.h"
...
DllExport int call_conv driverMySQLEmbedded_initialise()
{
	numHandles = 0;
	numQueries = 0;
	numPrepQueries = 0;
...
DllExport int call_conv driverMySQLEmbedded_lib_end()
{
...
DllExport int call_conv driverMySQLEmbedded_register(void)
{
	...
	registerXSBFunction("mysqlembedded", PREPARE, funcPrepare);
	...
	registerXSBFunction("mysqlembedded", EXEC_PREPARE, funcExecPrepare);
