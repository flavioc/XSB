...
\ouritem{thread\_create(+Goal,ThreadId,+OptionsList)}
...
\ouritem{thread\_create(+Goal,-ThreadId)}
\index{\texttt{thread\_create/2}}
%
Acts as {\tt thread\_create(Goal,ThreadId,[])}.

\ouritem{thread\_create(+Goal)}
\index{\texttt{thread\_create/1}}
%
Acts as {\tt thread\_create(Goal,\_,[detached(true)])}.

\ouritem{thread\_join(+Threads\_or\_aliases,-ExitDesignators)}
...
\ouritem{thread\_self(?ThreadId\_or\_Alias)}
...
\ouritem{thread\_detach(+Thread\_or\_Alias)}
...
\ouritem{thread\_cancel(+Thread\_or\_Alias)}
...
\ouritem{thread\_signal(Thread\_or\_Alias,Goal)}
\index{\texttt{thread\_signal/2}} 
% 
...
\ourmoditem{thread\_disable\_cancel/0}{thread}
\index{\texttt{thread\_disable\_cancel/0}} 
% 
...
\ourmoditem{thread\_enable\_cancel/0}{thread}
\index{\texttt{thread\_enable\_cancel/0}} 
% 
...
\ouritem{thread\_property(?ThreadOrAlias,?Property)}
\index{\texttt{thread\_property/2}} 
%
...
\ouritem{thread\_sleep(+Seconds)}
\index{\texttt{thread\_sleep/1}} 
%
...
\ourmoditem{usleep(+Microseconds)}{thread}
...
%
\begin{description}
\ouritem{message\_queue\_create(-Queue,+Options)}
...
\ouritem{message\_queue\_detroy(+Queue\_or\_Alias,\#Message)}
...
\ouritem{thread\_send\_message(+Queue\_or\_Alias,\#Message)}
...
\ouritem{thread\_get\_message(+Queue\_or\_Alias,?Message)}
...
\ei
%
\ouritem{thread\_get\_message(?Message)}
\index{\texttt{thread\_get\_message/1}} 
%
Acts as {\tt thread\_get\_message/2}, but on a thread's private queue.

\ouritem{thread\_peek\_message(+Queue\_or\_Alias,?Message)}
\index{\texttt{thread\_peek\_message/2}} 
%
If there are terms on {\tt Queue\_or\_Alias} {\tt
...
\ei
%
\ouritem{thread\_peek\_message(?Message)}
\index{\texttt{thread\_peek\_message/1}} 
%
...
\begin{description}
\ouritem{with\_mutex(+Mutex,?Goal)}
...
\ouritem{mutex\_property(?MutexOrAlias,?Property)}
