...
\begin{description}
\ourmoditem{throw(+ErrorTerm)}{ISO}\index{\texttt{throw/1}} 
%
...
%
\ourmoditem{domain\_error(+Valid\_type,-Culprit,+Predicate,+Arg)}{error\_handler}
...
\end{verbatim} }
%
\ourmoditem{evaluation\_error(+Flag,+Predicate,+Arg)}{error\_handler}
...
\end{verbatim} }
%
\ourmoditem{existence\_error(+Object\_type,?Culprit,+Predicate,+Arg)}{error\_handler}
...
%
\ourmoditem{instantiation\_error(+Predicate,+Arg,+State)}{error\_handler}
...
%
\ourmoditem{permission\_error(+Op,+Obj\_type,?Culprit,+Predicate)}{error\_handler}
...
%
\ourmoditem{representation\_error(+Flag,+Predicate,+Arg)}{error\_handler}
...
\end{verbatim} }
%
\ourmoditem{resource\_error(+Flag,+Predicate)}{error\_handler}
...
\end{verbatim} }
%
\ourmoditem{type\_error(+Valid\_type,-Culprit,+Predicate,+Arg)}{error\_handler}
...
%
\ourmoditem{misc\_error(+Message)}{error\_handler}
...
\begin{description}
\ourmoditem{catch(?Goal,?CatchTerm,+Handler)}{ISO} \index{\texttt{catch/3}}
...
handler.
%
\ourmoditem{error\_write(?Message)}{standard}\index{\texttt{error\_write/1}}
\vspace{-0.3in}
\ourmoditem{error\_writeln(?Message)}{standard}\index{\texttt{error\_writeln/1}}
...
\ourmoditem{close\_open\_tables}{machine}\index{\texttt{close\_open\_tables/0}}
%
...
\begin{description}
\ourmoditem{check\_atom(?Term,+Predicate,+Arg)}{error\_handler}
\index{\texttt{check\_atom/3}}
Checks that {\tt Term} is an atom.  If so, the predicate succeeds;
if not it throws a type error.

\ourmoditem{check\_ground(?Term,+Predicate,+Arg)}{error\_handler}
...
\ourmoditem{check\_integer(?Term,+Predicate,+Arg)}{error\_handler}
\index{\texttt{check\_integern/3}}
Checks that {\tt Term} is an integer.  If so, the predicate succeeds;
if not it throws a type error.

\ourmoditem{check\_nonvar(?Term,+Predicate,+Arg)}{error\_handler}
...
\ourmoditem{check\_var(?Term,+Predicate,+Arg)}{error\_handler}
\index{\texttt{check\_var/3}}
Checks that {\tt Term} is a variable.  If so, the predicate succeeds;
if not it throws an instantiation error.

\ourmoditem{check\_nonvar\_list(?Term,+Predicate,+Arg)}{error\_handler}
...
\ourmoditem{check\_stream(?Stream,+Predicate,+Arg)}{error\_handler}
...
\ourmoditem{check\_one\_thread(+Operation,+Object\_Type,+Predicate)}{error\_handler}
...
\begin{description}
\ourmoditem{xsb\_backtrace(-Backtrace)}{machine}
...
%
\ourmoditem{get\_backtrace\_list(+Backtrace,-PredicateList)}{error\_handler}
...
%
\ourmoditem{print\_backtrace(+Backtrace)}{error\_handler}
