...
cuts}\index{cut} (\cut)\index{{\tt "!/0}}, may be included in the {\em
...
\item The Prolog control primitives {\em if-then-else} ({\tt '->'/2}),
      ...
      {\em cut}\index{cut} (\cut)\index{{\tt "!/0}} may also be included in the 
...
term\_expansion/2}, \index{{\tt term\_expansion/2}} by means of which
...
\ouritem{tphrase(+Phrase)}\index{{\tt tphrase/1}} This predicate is
...
This predicate generally is of no concern to the user.  Rather it is used 
    ...
    {\em cuts}\index{cut} (\cut)\index{{\tt "!/0}}, or {\em if-then-elses} 
...
\ouritem{tphrase\_set\_string(+List)}\index{{\tt tphrase\_set\_string/1}}
This predicate 
...
\ouritem{tphrase\_set\_string\_keeping\_tables(+List)}\index{{\tt tphrase\_set\_string\_keeping\_tables/1}}
...
\ouritem{tphrase\_set\_string\_auto\_abolish(+List)}\index{{\tt tphrase\_set\_string\_auto\_abolish/1}}
This predicate is the same as {\tt tphrase\_set\_string}, except
it abolishes tables that have been indicated as dcg-supported tables
by a previous call to {\tt set\_dcg\_supported\_table/1}.

\ouritem{set\_dcg\_supported\_table(+TabSkel)}\index{{\tt set\_dcg\_supported\_table/1}}
