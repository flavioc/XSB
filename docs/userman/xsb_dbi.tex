...
\begin{itemize}
 ...
 MySQL from sources and configure MySQL using the {\tt --with-embedded-server} option.

 \item append to {\tt /etc/my.cnf} or $\sim${\tt/.my.cnf}:
       \begin{verbatim}
       ...
       {\tt /usr/local/mysql/share/mysql/english} with the directory
       appropriate for your MySQL installation.

       You might also need to set the {\tt datadir} parameter to specify
       the directory where the databases are to be kept. This has to be
       done if there is a possibility of running the embedded MySQL server
       alongside the regular MySQL server. In that case, the {\tt datadir}
       directory of the embedded server must be different from the {\tt
         datadir} directory of the regular server. This is because
       specifying the same directory might lead to the corruption of your
       databases. See
       {\tt http://dev.mysql.com/doc/refman/5.1/en/multiple-servers.html}
       for further details on running multiple servers.
\end{itemize}
