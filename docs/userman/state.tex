...
\item Global stack (heap) and local (environment) stack (see e.g.
  ...
  algorithm).  For maximum space used by each of these stacks in XSB's
  single-threaded configuration, use the {\tt '-s'} command-line
  option.
%
...
\item Information about the number of tabling operations performed in
  ...
  use (call) subsumptive tabling (see Section
  \ref{sec:TablingStrategies} and \cite{TST99}).
\begin{itemize}
...
\item Garbage Collection Information.  Time spent garbage collecting
  by all threads and number of heap cells collected.
...
list nodes (cf. \cite{SaSw98, RRSSW98, TST99}).
\end{itemize}
