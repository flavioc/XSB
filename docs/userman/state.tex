...
%
{\tt atom\_garbage\_collection} & {\tt on} if garbage collection for
...
\\ \hline
%
{\tt table\_gc\_action} & {\tt abolish\_tables\_transitively} if
  presicates or subgoals that depend on a conditional answer of an
  ...
  the process.\\ \hline 
%
{\tt goal} & the goal passed to XSB on command line with the `-e'
switch; `{\tt true.}' if nothing is passed.  This flag may be
examined, but not set. \\ \hline
%
