...
module implicitly used by the command line).  This can be done for all
predicates {\em except} {\tt {}/1} -- see below.  However, when
calling any of these predicates from compiled code, they must be
explicitly imported from their modules.
...
are checked for satisfiability.  {\em Note that from the command line,
  {\tt {}/2} must be called with the module prefix {\tt nf\_r}.  This
  requirement is intended to be temporary}.
...
\ournewitem{bb\_inf(+IntegerList,+Expr,-Inf,-Vertex, +Eps)}  {bb\_r}
%
...
\ournewitem{bb\_inf(+IntegerList,+Expr,-Inf)}  {bb\_r}
%
