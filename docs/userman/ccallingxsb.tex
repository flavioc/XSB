...
\end{small}
%
(cf. {\tt \$XSBDIR/examples/c\_calling\_xsb/edb.P}).  
...
are discussed in Section~\ref{}.  Finally, we show how to a C program
can call and manage multiple XSB threads in Section~\ref{}.

\subsection{The XSB API for the Sequential Engine Only} \label{sec:CXSBSeq}

Figure~\ref{fig:varstringex} shows a simple program to backtrack
through the {\tt p/3} predicate above to do this, and can be found in
{\tt \$XSBDIR/examples/c\_calling\_xsb/cvarstring.c}).  This example
will only compile properly if the sequential engine is used, and its
style is not recommended: it will be shown in Section~\ref{} how to
extend the style.
%
\begin{figure}[hbtp]
\begin{small}
...
is needed for string manipulation routines such as {\tt strcpy}.  In
addition, the XSB library header {\tt cinterf.h} is necessary for the
XSB C API.  Within {\tt main()} since the program in
...
1 {\em Exception Handling} in that it has both a {\em type} and a {\em
  message}.  For normal Prolog exceptions, XSB's API will throw the
same kinds of errors as XSB called in a stand-alone or server mode,
i.e. instantiation errors, type errors, etc.  However XSB's API adds
two new error types:
%
\begin{itemize}
...
\end{itemize}
%
Errors raised by {\tt xsb\_init\_string()} always have type {\tt
  init\_type}, and a string pointer to the associated message can be
found by the function {\tt xsb\_get\_init\_error\_message()}.  As a
...
be obtained by {\tt xsb\_get\_error\_message()}.  The reason that
initialization errors are handled separately is discussed in
Section~\ref{}.  Note, however that the argument to {\tt
  xsb\_command\_string} must be a syntactically valid Prolog term
ending with a period, otherwise a syntax error will be thrown, which
may be displayed through {\tt xsb\_get\_error\_type()} and {\tt
...
\subsection{Multi-threaded Programming with the XSB API} \label{sec:CXSB}
...
query cXSB in a mechanism reminiscent of database cursors.
...
loop as the main thread, and sets {\tt r\_th} to a pointer to the
context of the new thread.  The new thread {\tt r\_th} can be used for
...
the command loop, it will stay around until it is explicitly exited or
until XSB is closed.  A call such as 
\begin{verbatim}
xsb\_command\_string(r_th,"thread_exit.")
\end{verbatim}
...
\begin{description}
\ouritem{int xsb\_command\_string(th\_context *th, char *cmd)} \index{\texttt{xsb\_command\_string}}
%
...
\begin{description}
\ouritem{char * xsb\_get\_init\_error\_message()} \index{xsb\_get\_error\_message}
%
...
\ouritem{char * xsb\_get\_error\_type(th\_context *th)} \index{xsb\_get\_error\_type}
%
...
\ouritem{char *xsb\_get\_error\_message(th\_context *th)}\index{xsb\_get\_error\_message}
%
If a function called for {\tt th} returned {\tt XSB\_ERROR} this
...
\ouritem{int xsb\_ccall\_thread\_create(th\_contex *callingThread, th\_context **newThrea)}
\index{xsb\_ccall\_thread\_create}
%
...
\ouritem{th\_context *xsb\_get\_main\_thread()}\index{xsb\_get\_main\_thread}
%
...
\index{xsb\_thread\_id\_to\_context}

\ouritem{th\_context *xsb\_thread\_context\_to\_id(xsb\_tid id)}
\index{xsb\_thread\_context\_to\_id}
