...
The next level provides routines that return answers with an interface
that does not require complex C types.  The routines at this level
are: {\tt
xsb\_query\_string\_string\_b(query,buff,bufflen,anslen,sep)}, {\tt
xsb\_next\_string\_b(buff,bufflen,anslen,sep)}, and {\tt
xsb\_get\_last\_answer(buff,bufflen,anslen)}.  They are normally
intended to be used with the routines above.
