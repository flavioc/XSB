...
\index{\texttt{library\_directory/1}} 
...
\item Dynamic loading does not allow module/export declarations, mode
  ...
  files to import predicates, allows tabling declarations (except for
  {\tt auto\_table} and {\tt suppl\_table}, operator declarations
  (when a canonical read is not used) and declarations of a predicate
  to be thread\_shared or thread\_private.  \ei \ei
...
%--------------------------------------
\item[{\tt spec\_repr}]\index{\texttt{spec\_repr}} When specified, the
  ...
  calls.
\item[{\tt spec\_off}]\index{\texttt{spec\_off}} When specified, the
  compiler does not perform specialization of partially instantiated
  calls.
\item[{\tt unfold\_off}]\index{\texttt{unfold\_off}} When specified,
...
%qtls ??
\item[{\tt spec\_dump}]\index{\texttt{spec\_dump}} Generates a {\tt
