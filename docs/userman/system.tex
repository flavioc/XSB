...
Finally, if you developed and tested  a package that you think is
...
cmd_loop_driver:
   ...
   -g gc_type : choose the garbage collection ("none", "indirection", sliding", or "copying") 
...
\item[{\tt -d}] Produces a disassembled dump of {\tt byte\_code\_file} to 
    {\tt stdout} and exits.
\item[{\tt -c} {\em size}] Allocates {\em initial  size\/} KB of space
...
collectors, and \cite{CaSC01} for a description of the third).  The
choice {\tt indirection} is not available on CHAT, and this choice
will be ineffective on emulators configured for CHAT.
%
