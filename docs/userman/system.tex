...
To invoke XSB's top level interpreter (the usual way of
using XSB), simply issue the command:
\begin{verbatim}
                       $XSB_DIR/bin/xsb -i
\end{verbatim}
...
UNIX or Windows command line.  As long as {\tt foo.P} contains a
...
fact the command \verb'$XSB_DIR/bin/xsb -i' is equivalent to the
command:
\begin{verbatim}
...
\ref{emulator_options}), and double their size until it is not
...
retracted clauses, and for the heap, when the {\tt --enable-chat}
option is used.
