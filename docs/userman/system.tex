...
Unlike some Prologs, however, there is no difference between compiled
and consulted code: ``Compiling'' in XSB means creation of a file
containing SLG-WAM byte-code; ``Consulting'' means loading such a
byte-code file, after compiling it (if the source file was altered
later than the object file).
...
prefix and the extension {\tt .P}, {\tt .pl} or other filenames, see
Section~\ref{sec:filenames} to {\tt File}.  The directory prefix must
...
in a ($\langle$filename$\rangle$.xwam), but {\tt compile/[1,2]} does
{\em not\/} load the object file it creates.  (The standard predicates
{\tt consult/[1,2]} and {\tt reconsult/[1,2]} both load the object
file into the system, after recompiling the source file if needed.)
The object file created is always written into the directory where the
source file resides: the user must therefore have write permission in
that directory.
...
\begin{description}
\item[{\tt optimize}]\index{\texttt{optimize}} When specified, the
...
\item[{\tt singleton\_warnings\_off}] Does not print out any warnings
  ...
  other program.
%
v\item[{\tt xpp\_on}]\index{\texttt{xpp\_on}}\index{GPP}
