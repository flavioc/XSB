...
\item[{\tt cpp\_on}]\index{{\tt cpp\_on}}
  Filter the program through the C preprocessor before sending it to the XSB
  ...
  XSB directs the C preprocessor to search for the files to include in the
  directories \verb|$XSB_DIR/emu| 
  %%$
  and \verb|$XSB_DIR/prolog_includes|.
  %%$
  \index{\tt xsb\_cpp\_include\_dir}
  %%
  However, additional directories can be added to this search path by
  asserting into the predicate \verb|xsb_cpp_include_dir/1|, which should
  ...
  %%
  \begin{itemize}
    \item  {\it Quotes\/}: C requires that quotes come in pairs, which is a
      ...
      this, it might refuse to apply \verb|#define| macros to anything between
      ...
      a comment of the following form at the end of the line as follows: 
      %%
\begin{verbatim}
            put(0'p),       %  here's that other quote
\end{verbatim}
      %%
      Another way to circumvent the problem is to use \verb|#define|,
      {\it e.g.}, 
      %%
\begin{verbatim}
    #define CH_P  80
...
\end{verbatim}
      %%
    \item {\it Comments\/}: The C preprocessor does not understand Prolog
