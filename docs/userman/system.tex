...
\index{modules!XSB syntax}
\index{{\tt export/1}} \index{{\tt import/2}} \index{{\tt local/1}}
...
\index{{\tt module/2}}  \index{{\tt use\_module/2}}    
{\bf Compatability Note:\ }  
% 
Unlike XSB, most other Prologs use a predicate-based module system
that differs from XSB's module system in syntax and in certain
semantic aspects.  Unlike the semantic issues, some syntactic issues
can be easily addressed.  The declaration
...
\noindent
as long as the module name is the same as the file name, $filename$.
Similarly,
...
\noindent
Accordingly, {\tt use\_module/2} and {\tt module/1} can be used just as
{\tt import/2} and {\tt export/1}.  However the common declaration

\demo{:- use\_module($module$).}

\noindent
`is {\em not} equivalent to an XSB import statement, as each XSB import
statement must explicitly declare a list of predicates that are used
from each module.
