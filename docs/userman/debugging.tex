...
\ouritem{debug\_ctl(option,value)} \index{{\tt debug\_ctl/2}}
   ...
   %%
   \begin{enumerate}
    \item {\tt debug\_ctl(skip, print)}
      Set verbose skip globally. This essentially means that trace
      will always be printed and the user will never be prompted during the
      trace.
    \item {\tt debug\_ctl(skip, noprint)} 
      Turn global verbose skip mode off.
