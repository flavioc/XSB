...
\ouritem{write(?Term)}\index{{\tt write/1}}
    ...
    {\tt numbervars/[1,3]} described in Section~\ref{NumberVars}\@.
%% $
...
\section{Negation and Control}\index{control}
...
\ouritem{is\_string(?X)}\index{{\tt is\_string/1}}
...
\ouritem{name(?Constant, ?CharList)}\index{{\tt name/2}} 
    ...
    standard predicate {\tt atom\_chars/2} should be used instead. The 
    ...
    the predicates {\tt atom\_chars/2} and {\tt number\_chars/2} described
    below.
...
\ouritem{atom\_chars(?Atom, ?CharList)}\index{{\tt atom\_chars/2}} 
    The standard predicate {\tt atom\_chars/2} performs the conversion 
    ...
    If both of the arguments of {\tt atom\_chars/2} are uninstantiated or
    {\tt CharList} is not a proper list of ASCII characters, {\tt
    atom\_chars/2} aborts, and an error message will be sent to
    ...
    {\footnotesize
     \begin{verbatim}
                   ...
                   | ?- atom_chars(X, [52,51,49]), integer(X).
                   no

                   | ?- atom_chars(X, [52,Y,49]).
                   ! Instantiation error in argument 2 of atom_chars/2
                   ! Aborting...

                   | ?- atom_chars(431, L).
                   ! Type error: in argument 1 of atom_chars/2
                   ! atom expected, but something else found
                   ! Aborting...

                   | ?- atom_chars(X, [52,300,49]).
                   ! Range error: in argument 2 of atom_chars/2
...
\ouritem{number\_chars(?Number, ?CharList)}\index{{\tt number\_chars/2}} 
    The standard predicate {\tt number\_chars/2} performs the conversion 
    ...
    {\tt number\_chars/2} will simply fail.

    If both of the arguments of {\tt number\_chars/2} are uninstantiated or
    {\tt CharList} is not a proper list of ASCII characters, {\tt
    number\_chars/2} aborts, and an error message will be sent to
    the standard error stream.
    ...
    {\footnotesize
     \begin{verbatim}
                   ...
                   | ?- number_chars(N, [49,50,51]), integer(N).
                   N = 123

                   | ?- number_chars(31.4e+10, L).
                   L = [51,46,49,51,57,57,57,55,69,43,49,48]

                   | ?- number_chars(N, "314e+8").
                   N = 3.14e+10
...
\index{{\tt bagReduce/4}}
    ...
    HiLog predicate.  {\tt Op} must be a Hilog operation, i.e., a 3-ary
    HiLog predicate that defines an associative operator.  The predicate
    must define a binary function in which the first two arguments
    determine the third.  {\tt Id} must be the identity of the operator.
    {\tt bagReduce} returns with {\tt Arg} bound to the ``reduce'' of the
    elements of the bag determined by {\tt SetPred} under the operation {\tt
    Op}.  I.e., {\tt Arg} becomes the result of applying the operator to
    all the elements in the bag that unify with {\tt Arg}.  See the {\tt
    bagSum} operator below to see an example of {\tt bagReduce}'s use.
...
\index{{\tt bagPO/4}}
    ...
    See the {\tt bagMax} operator below to see an example of {\tt bagPO}'s use.
