...
\ournewitem{filterReduce(?SetPred,?Arg,+Op,+Id)}{HiLog,Tabling}
\index{{\tt bagReduce/4}} 
...
\ournewitem{filterPO(?SetPred,?Arg,+Order)}{HiLog,Tabling}
\index{{\tt bagPO/4}}
\index{{\tt filterPO/4}}
...
{\tt filterPO/4} acts as {\tt bagPO/4} with the single difference that
it does not depend on HiLog, so that {\tt filterPO/4} will be more
