...
\ournewitem{sk\_not(+P)}{Tabling}\index{{\tt sk\_not/1}}
    Same as {\tt tnot/1} but does not check for floundering.  Thus it
    permits variables in its subgoal argument and gives the semantics
    similar to Prolog's when there are variables.  (This replaces the
    {\tt 't not'/1} predicate of earlier XSB versions whose
    implementation and semantics were dubious.)    
