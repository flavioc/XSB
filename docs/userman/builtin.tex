...
and userout (the user accesses them both via the name {\tt ``user''},
...
\begin{description}
\ournewitem{set\_input(+F)}{ISO}\index{{\tt set\_input/1}}
\vspace{-7mm} 
...
\ournewitem{set\_output(+F)}{ISO}\index{{\tt set\_output/1}}
\vspace{-7mm} 
...
\ournewitem{open(+File,+Mode,-Stream)}{ISO}\index{{\tt open/3}}
...
\ournewitem{open(+File,+Mode,-Stream.+Options)}{ISO}\index{{\tt open/4}}
...
\ournewitem{close(+Stream)}{ISO}\index{{\tt close/1}}
	{\tt close/1} closes the stream {\em Stream}.

\ournewitem{close(+Stream.+Options)}{ISO}\index{{\tt close/2}}
...
\ouritem{get\_code(?N)}\index{{\tt get\_code/1}}
\vspace{-7mm} 
\ouritem{get\_code(+Stream,?N)}\index{{\tt get\_code/2}}
...
\ouritem{get\_char(?Char)}\index{{\tt get\_char/1}}
\vspace{-7mm} 
\ouritem{get\_char(+Stream,?Char)}\index{{\tt get\_char/2}}
...
\ouritem{put\_code(+N)}\index{{\tt put\_code/1}}
\vspace{-7mm} 
\ouritem{put\_code(+Stream,+N)}\index{{\tt put\_code/2}}
    Puts the ASCII character code {\tt N} to the current output stream
    or to {\tt Stream}.  The semantics of these predicates are based
    on that of {\tt put/1} and do not conform in all cases to the ISO
    standard.

\ouritem{put\_char(+Char)}\index{{\tt put\_char/1}}
\vspace{-7mm} 
\ouritem{put\_char(+Stream,+Char)}\index{{\tt put\_char/2}}
...
%--------
\ouritem{read\_term(?Term,?OptionsList)}\index{{\tt read\_term/2}}
%
...
\ouritem{read\_term(+Stream, ?Term,?OptionsList)}\index{{\tt read\_term/3}}
%
...
\ouritem{write\_term(?Term,+Options)}\index{{\tt write\_term/2}}
\vspace{-7mm} 
\ouritem{write\_term(+Stream, ?Term,+Options)}\index{{\tt write\_term/3}}
...
\begin{description}
\ouritem{is(?Result,+Expression)}\index{{\tt is/2}} 
...
\begin{description}
\ournewitem{+(+Expr1,+Expr2)}{Evaluable Functor}\index{{\tt +/2}} 
...
\ournewitem{-(+Expr1)}{Evaluable Functor}\index{{\tt -/1}} If {\tt
+Expr} evaluates to {\tt Number}, returns {\tt -Number1},
performing any necessary type conversions.

\ournewitem{'/\'(+Expr1,+Expr2)}{Evaluable Functor}\index{{\tt
'/\'/2}} If {\tt +Expr1} evaluates to {\tt Number1}, and {\tt Expr2}
...
\ournewitem{ceiling(+Expr)}{Evaluable Functor}\index{{\tt ceiling/1}}
...
\ournewitem{float(+Expr)}{Evaluable Functor}\index{{\tt float/1}} If
...
\ournewitem{floor(+Expr)}{Evaluable Functor}\index{{\tt floor/1}} If
...
\ournewitem{mod(+Expr1,+Expr2)}{Evaluable Functor}\index{{\tt mod/2}}
If {\tt +Expr1} evaluates to {\tt Number1} and {\tt Expr2} evaluates
...
\ournewitem{rem(+Expr1,+Expr2)}{Evaluable Functor}\index{{\tt rem/2}}
If {\tt +Expr1} evaluates to {\tt Number1} and {\tt Expr2} evaluates
to {\tt Number2} where {\tt Number2} is not 0, {\tt
...
\ournewitem{round(+Expr)}{Evaluable Functor}\index{{\tt round/1}} If
{\tt +Expr} evaluates to {\tt Number}, {\tt round(Number)} returns the
nearest integer to {\tt Number} if {\tt Number} is a float, and {\tt
Number} itself if {\tt Number} is an integer.

\ournewitem{sqrt(+Expr)}{Evaluable Functor}\index{{\tt sqrt/1}}
If {\tt +Expr} evaluates to {\tt Number}, {\tt sqrt(Number)}
returns the square root of {\tt Number}.

\ournewitem{truncate(+Expr)}{Evaluable Functor}\index{{\tt
truncate/1}} If {\tt +Expr} evaluates to {\tt Number}, {\tt
...
\index{{\tt cos/1}} \index{{\tt sin/1}} \index{{\tt tan/1}}
\index{{\tt acos/1}} \index{{\tt asin/1}} \index{{\tt atan/1}}
\index{{\tt log/1}} \index{{\tt log10/1}}
...
\begin{description}
\ouritem{\cut}\index{{\tt "!/0}}  \index{cut} 
...
\ouritem{$\backslash$+ +P}\index{{\tt $\backslash$+/1}}
...
%
    ...
    \ournewitem{sk\_not(+P)}{Tabling}\index{{\tt sk\_not/1}} 
%% 
...
\ouritem{is\_attv(+Term)}\index{{\tt is\_attv/1}}
    Succeeds is {\tt Term} is an attributed variable, and fails otherwise.

\ouritem{is\_most\_general\_term(?X)}\index{{\tt
is\_most\_general\_term/1}}
...
\ouritem{atom\_codes(?Atom, ?CharCodeList)}\index{{\tt atom\_codes/2}} 
...
\ouritem{atom\_chars(?Number, ?CharAtomList)}\index{{\tt atom\_chars/2}} 
...
\ouritem{number\_codes(?Number, ?CharCodeList)}\index{{\tt number\_codes/2}} 
...
\ouritem{number\_chars(?Number, ?CharAtomList)}\index{{\tt number\_chars/2}} 
...
\ouritem{number\_digits(?Number, ?DigitList)}\index{{\tt number\_digits/2}} 
...
\ouritem{bagof(?X, +Goal, ?Bag)} \index{{\tt bagof/3}}
...
\ouritem{findall(?X, +Goal, ?List)} \index{{\tt findall/3}}
...
%\predindex{call/1~(P)}
    ...
    {\em except that} any cut (`!')\index{{\tt "!/0}}\index{cut}
