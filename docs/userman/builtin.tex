...
XSBs I/O is based on ISO-style streams, although it also supports
older DEC-10 style file handling.  XSB's I/O streams are small
integers, but should not be confused with the file descriptors used by
the OS.  The OS file descriptors are objects returned by the C {\tt
open} function; XSB I/O streams indices into the internal XSB table of
open files and associated information. The OS does not know about XSB
I/O streams, while XSB (obviously) does know about the OS file
descriptors. An OS file descriptor may be returned by certain
predicates (e.g.  {\tt pipe\_open/2} or user-defined I/O).  In such a
file descriptor can be promoted to XSB stream using the predicate {\tt
fd2iostream/2}.

% TLS: refer to it in Manual 2.

The default input and output streams can be denoted by {\tt user\_in}
and {\tt user\_out} and they refer to the process'es standard input
and standard output streams) \footnote{For backwards compatability,
the default input stream can also be aliased by {\tt user} or {\tt
userin}, and the default output stream by {\tt user} or {\tt
userout}.}.

\subsection{I/O Stream Implementation}

Typically XSB opens files for buffered I/O, so XSB streams internally
refer to {\tt FILE} data structures (those returned by the C {\tt
fopen} function).  When it starts, XSB opens a number of standard I/O
streams that it uses to print results, errors, debugging info, etc. The
descriptors are described in the file {\tt
prolog\_includes/standard.h}. This file provides the following
...
{\bf Error Cases}
\bi
\item 	{\tt File} or {\tt Mode} is not instantiated
\bi
...
\ei
%
\item 	{\tt File} cannot be opened, or opened in the desired mode
\bi
\item 	{\tt permission\_error(open,file,File)}
\ei
\ei
...
options to be given.  The only effective option in \version{} of XSB
is {\tt alias(A)} which allows the stream to be aliased to an atom.
The ISO option {\tt reposition(Boolean)} currently has no effect on
file streams, which are always repositionable.  The ISO option {\tt
type(T)} currently has no effect on file streams, which are always
textual.  And finally, the ISO option {\tt eof\_action(Action)}
currently has no effect on file streams.  Appropriate warnings are
given if an option list includes these option types.
...
\ournewitem{close(+Stream\_or\_alias.+OptionsList)}{ISO}\index{\texttt{close/2}}
	{\tt close/2} closes the stream or alias {\tt
	Stream\_or\_alias}.  {\tt OptionsList} allows the user to
	declare whether a permission error will be raised in XSB upon
	a resource or system error from the closing function
	(e.g. {\tt fclose()} or other system function).  If {\tt
	OptionsList} is non-empty and contains only terms of the form
	{\tt force(true)} then such an error will be ignored (possibly
	leading to unacknowledged loss of data).  If {\tt OptionsList}
	contains terms of the form {\tt force(true)} and {\tt
	force(false)}, then an error is raised.  Otherwise the
	``default'' behavior is to raise a permission error.
...
Behaves as the ``default'' case {\tt
close(Stream\_or\_alas,[force(false)])}.
...
\ournewitem{stream\_property(?Stream\_or\_alias,?Property)}{ISO}
\index{\texttt{stream\_property/2}}
%
This predicate backtracks through the various stream properties that
unifiy with {\tt Property} for the stream {\tt Stream\_or\_alias}.  Currently,
the following properties are defined.

\bi
\item {\tt file\_name(F)} is a property of {\tt Stream\_or\_alias}, if {\tt F} is
the file name associate with {\tt Stream\_or\_alias}.  The full operating system
path is used.
%
\item {\tt mode(M)} is a property of {\tt Stream\_or\_alias}, if {\tt M} represents the
I/O mode: {\tt read}, {\tt write}, {\tt append}, {\tt read\_plus},
{\tt write\_plus}, or {\tt append\_plus} with which {\tt Stream\_or\_alias} was
opened.
%
\item {\tt input}  is a property of {\tt Stream\_or\_alias}, if {\tt
Stream\_or\_alias} was opened in the I/O mode: {\tt read}, {\tt
read\_plus}, {\tt write\_plus}, or {\tt append\_plus}.
% 
\item {\tt output}  is a property of {\tt Stream\_or\_alias}, if {\tt
Stream\_or\_alias} was opened in the I/O mode: {\tt write}, {\tt
append}, {\tt read\_plus}, {\tt write\_plus}, or {\tt append\_plus}.
%
\item {\tt alias(A)}  is a property of {\tt Stream\_or\_alias}, if
{\tt Stream\_or\_alias} was opened with alias {\tt A}.
%
\item {\tt reposition(Bool)} is true, if {\tt Stream\_or\_alias} is
repositionable, and false otherwise.  In XSB \version, user streams
are always repositionable, although system streams may not be.
%
\item {\tt type(Type)} is true, if {\tt Stream\_or\_alias} is of type
{\tt text} or {\tt binary}.  In XSB \version{} all streams are text
streams.
%
% end_of_stream.
\end{itemize}
...
\index{\texttt{flush\_output/1}}
    Any buffered data in {\tt Stream\_or\_aias}  gets flushed.
...
\bi
\item 	{\tt permission\_error(flush,stream,Stream\_or\_alias)}
\ei
\ei
...
\index{\texttt{set\_stream\_position/2}}
    Sets the stream position indicator for the next input or output
    operation. Position is a term {\tt position(Place,Offset)}.  The
    value of {\tt Place} can be {\tt start\_of\_file}, {\tt
    current\_pos}, or {\tt end\_of\_file}, which correspond to the
    beginning of the file, the current position in the file, or the
    end of the file, respectively. {\tt Offset} is a positive or
    negative integer denoting the offset from {\tt Place}.
...
\item 	{\tt domain\_error(stream\_or\_alias,Stream\_or\_alias)}
\ei
\item 	{\tt Position} is not instantiated to a position term as described above.
\bi
...
\item 	{\tt existence\_error(stream,Stream\_or\_alias)}
\ei
\item 	Repositioning returns an error.
\bi
...
%
Succeeds if {\tt Stream\_or\_alias} has position at or past the end of stream.
...
%
Succeeds if the current stream has position at or past the end of stream.
...
\ournewitem{file\_truncate(+Stream, +Length, -Return)}{file\_io}
...
\index{\texttt{file\_clone/1}}
    ...
    variable, then this call creates a new XSB I/O stream that is a
    clone of {\tt SrcStream}. This means that I/O sent to either
    descriptor goes to the same place. If {\tt DestStream} is not a
    variable, then it must be a number corresponding to a valid I/O
    stream. In this case, XSB closes {\tt DestStream} and makes it 
    into a clone on {\tt SrcStream}. For instance, suppose that 10 is a
    I/O Stream that is currently open for writing to file {\tt foo.bar}.
    Then
    %%
    \begin{verbatim}
    | ?- file_clone(10,3,_).      
...
\ouritem{read\_line\_list(-String)}
\index{\texttt{read\_line\_list/1}}
...
avoids interning an atom as does {\tt read\_line\_atom/3}, and so is
recommended when speed is important.  This predicate fails on reaching
the end of file.

\ouritem{read\_line\_list(Stream\_or\_alias,-CharList)}
\index{\texttt{read\_line\_list/2}}
Acts as does {\tt read\_line\_list}, but uses {\tt Stream\_or\_atom}.
...
\ouritem{read\_line\_atom(-Atom)}
\index{\texttt{read\_line\_atom/1}}
%
...
\ouritem{read\_line\_atom(+Stream\_or\_alias,-Atom)}
\index{\texttt{read\_line\_atom/2}}
Like {\tt read\_line\_atom/1} but reads from {\tt Stream\_or\_alias}.
%
{\bf Error Cases} 
\bi
\item 	{\tt Stream\_or\_alias} is not instantiated to a stream term
...
\ournewitem{write\_line(+String, +Offset)}{file\_io}
\index{\texttt{write\_line/2}}
...
\ournewitem{write\_line(+Stream\_or\_alias, +String, +Offset)}{file\_io}
\index{\texttt{write\_line/3}}
...
\ouritem{file\_read\_line\_atom(+IOport,-String)}
...
\ouritem{file\_read\_line\_atom(-String)}
\index{\texttt{file\_read\_line\_atom/1}}
Like \verb|file_read_line_atom/2|, but uses the current input stream.

\ournewitem{file\_write\_line(+IOport, +String, +Offset)}{file\_io}
\index{\texttt{file\_write\_line/3}}
   Write {\tt String} beginning with character {\tt Offset} to the
   output file represented by the I/O port {\tt IOport}. {\tt String}
   can be an atom or a list of ASCII character codes. This does
   \emph{not} put the newline character at the end of the string
   (unless {\tt String} already had this character). Note that escape
   sequences, like \verb|\n|, are recognized if {\tt String} is a
   character list, but are output as is if {\tt String} is an atom.
...
\ournewitem{getbuf\_list(+Stream\_or\_alias, +BytesRequested, -CharList, -BytesRead)}{file\_io}
\index{\texttt{getbuf\_list/4}}
Read {\tt BytesRequested} bytes from file represented by {\tt
Stream\_or\_alias} (which must already be open for reading) into
...
\ournewitem{getbuf\_list(+BytesRequested, -String, -BytesRead)}{file\_io}
\index{\texttt{getbuf\_list/3}}

Like \verb|file_getbuf_list/3|, but reads from the currently open input stream
({\it i.e.}, with {\tt see/1}).


\ournewitem{getbuf\_atom(+Stream\_or\_alias, +BytesRequested, -String, -BytesRead)}{file\_io}
\index{\texttt{file\_getbuf\_atom/4}}
Read {\tt BytesRequested} bytes from file represented by {\tt
Stream\_or\_alias} (which must already be open for reading) into
variable {\tt String}. This is analogous to {\tt fread} in C.  This
predicate always succeeds. It does not distinguish between a file
...
\ournewitem{getbuf\_atom(+BytesRequested, -String, -BytesRead)}{file\_io}
