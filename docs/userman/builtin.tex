...
debugging information is written, and ???.  All of these streams are
aliased by default to standard output, and can be redirected by the
predicates the predicates {\tt file\_reopen/4} and {\tt
file\_clone/3}.
...
the predicate {\tt term\_to\_atom/2} contains some of the
functionality that such streams would provide.
%
\item {\tt Pipe} XSB can also open piles either directly, or as part
of its ability to spawn processes.  Pipes are textual and not
repositionable.
\ei
...
pipe\_open/2} or user-defined I/O).  In such a file descriptor can be
promoted to XSB stream using the predicate {\tt fd2iostream/2}.
...
{\tt OptionsList} is non-empty and contains only terms of the form
{\tt force(true)} then such an error will be ignored (possibly leading
to unacknowledged loss of data).  If {\tt OptionsList} contains terms
of the form {\tt force(true)} and {\tt force(false)}, then an error is
raised.  Otherwise the ``default'' behavior is to raise a permission
error.

If the stream class of {\tt Stream\_or\_alias} is an atom, then the
only action taken is to close the stream itself -- the interned atom
...
i.e. {\tt file}, {\tt atom}, etc.

\item {\tt file\_name(F)} is a property of {\tt Stream\_or\_alias}, if
{\tt Stream\_or\_alias} is a file stream and {\tt F} is the file name
associate with {\tt Stream\_or\_alias}.  The full operating system
path is used.
%
\item {\tt type(T)} is a property of {\tt Stream\_or\_alias}, if
{\tt Stream\_or\_alias} is a file stream and {\tt T} is the file type
of {\tt Stream\_or\_alias}: {\tt text} or {\tt binary}.
%
\item {\tt mode(M)} is a property of {\tt Stream\_or\_alias}, if {\tt
M} represents the I/O mode with which {\tt Stream\_or\_alias} was
...
Stream\_or\_alias} was opened in the I/O mode: {\tt read}.
% 
...
{\tt Stream\_or\_alias} is not buffered (i.e. if it is of class {\tt
...
operation. Position is a term {\tt position(Place,Offset)}.  The value
of {\tt Place} can be {\tt start\_of\_file}, {\tt current\_pos}, or
{\tt end\_of\_file}, which correspond to the beginning of the file,
the current position in the file, or the end of the file,
respectively. {\tt Offset} is a positive or negative integer denoting
the offset from {\tt Place}.
...
\portability This predicate is not defined on Windows unless compiled
using Cygwin.
