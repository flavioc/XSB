...
\paragraph*{Failure to Detect a Suitable C Preprocessor}
XSB filters many of its system files (even its Prolog programs) through the
C preprocessor. (User programs can be also filtered this way using a
compilation option.) The configuration script tries to detect a suitable C
preprocessor. In a small number of cases, however, the preprocessor it
finds might not coexist with the XSB compiler. In such a case, you must
find a preprocessor that can be told the following:
%%
\begin{itemize}
  \item  Not to insert any lines that start with the hash sign \# (usually
    {\tt -P}). 
  \item To preserve the comments (usually {\tt -C}).
  \item To undefine any non-tandard symbols (usually {\tt -undef}).
  \item To take files that do not necessarily end with .c or .h.
    For gcc, this can be done using the \mbox{\tt -x c} flag. Most other
    preprocessors will take any file without any flags.  
\end{itemize}
%%
The configure script can be told with preprocessor to use using the {\tt
  --with-cpp} option.  For example,
%%
\begin{verbatim}
    configure --with-cpp="gcc -E -P -C -undef -x c"
    configure --with-cpp="/lib/cpp -P -C -undef"  
\end{verbatim}
%%
(Of course, if you have gcc or /lib/cpp, the configure script will find
them automatically.) If you system does not have a suitable preprocessor,
we suggest that you download gcc or lpp.\footnote{
  %%
  The SUNPro {\tt cc} compiler is an example of a preprocessor that is
  incompatible with XSB, because even if one invokes it with all the
  right options, like {\tt cc -E -P -C -undef}, it will still insert lines
  that begin with \#, and the XSB compiler will choke on them. Fortunately,
  Sun systems usually have a well-behaving preprocessor, such as
  {\tt /usr/ccs/lib/cpp} or {\tt gcc}.
  %%
  }
