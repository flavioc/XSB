...
have problems in \version{}.  Typically, if the 64-bit system
generates 32-bit code by default, XSB will run just as in 32-bit mode.
64-bit compilation can be forced for XSB by configuring with the
option {\tt --with-bits64}, and in a similar manner 32-bit compilation
can be forced with the option {\tt --with-bits32}.  XSB works well
under these configurations {\em except} that the interfaces may not
always work smoothly when C calls XSB or XSB calls C.  However, if the
default compilation is used, both the XSB calling C and the C calling
XSB interfaces to work properly.  If any of these limitations prove a
