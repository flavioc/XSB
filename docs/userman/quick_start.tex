...
\item
 ...
 \item Click on {\bf Insert}, then click on {\bf Files into Project ...}.
   Select all files from the {\tt emu} directory (highlight the
   {\tt*.[chi]} files) and the {\tt .h} files from the $\tt emu\backslash
   debugs$ and $\tt emu\backslash configs$ directories. Add them to the
   project (click {\bf Add}). Do not include the files in the {\tt dde}
   subdirectory of {\tt emu}.  
...
\item
   ...
   up):
	...
	(WIN32, and \_CONSOLE are already there):
	{\tt DIR=$\backslash$"C:$\backslash
	\backslash$xsbsys$\backslash \backslash$xsb$\backslash$"} (with
	your xsb directory, required), 
	{\tt ORACLE} (for Oracle interface, if
	desired), {\tt XSB\_ODBC} (for ODBC interface, if desired), {\tt
	XSB\_DLL} (if you are compiling XSB as a DLL).
	...
	odbccp32.lib} are already there): {\tt wsock32.lib}
	and the full path to an SQL lib (for Oracle support,
	e.g., {\tt
...
You should see the XSB executable in the directory
%%
\begin{equation}
 \tt
