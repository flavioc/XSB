...
denoted by the command is to be compiled using tabling.  Executed as a
directive in a state of execution where no clauses exist for each
denoted predicate {\tt table/1} ensures that any clauses asserted
each predicate use tabling.
{\tt Operations} can take one of two forms:
%
\begin{enumerate}
...
\item {\tt subsumptive} which causes the table evaluation method of
  the predicate(s) to use call subsumption, and is
  equivalent to {\tt ?- use\_subsumprive\_tabling PredSpec}.
%
\item {\tt variant} which causes the table evaluation method of the
  predicate(s) to use call variance, and is equivalent to {\tt ?-
    use\_variant\_tabling PredSpec}.
%
\item {\tt incremental} which causes the table evaluation method of
  the predicate(s) to be incremental and is equivalent to {\tt ?-
    use\_incremental\_tabling PredSpec}.
%
\item {\tt opaque} which indicates that the tables predicate is used
  ...
  incrementally maintained themselves.  It is equivalent to {\tt ?-
    use\_opaque\_tabling PredSpec}.
%
\item {\tt private} which causes the predicate(s) to be treated as
  thread private in addition to being tabled, and is equivalent to
  {\tt ?- thread\_private PredSpec}.
%
\item {\tt shared} which causes the predicate(s) to be treated as
  thread shared in addition to being tabled, and is equivalent to
  {\tt ?- thread\_shared PredSpec}.
%
\ei
\ei
\end{enumerate}
...
{\bf Error Cases}
\bi
\item {\tt Operations} is not ground
\bi
\item {\tt instantiation\_error}
\ei
\item {\tt Operations} is of the form {\tt PredSpec} as {\tt Options} and 
\bi
\item {\tt PredSpec} is not a comma-list 
\bi
\item {\tt domain\_error(comma\_list\_or\_predicate\_indicator,PredSpec)}
\ei
\item {\tt Options} is not a table option or a comma-list of table options
\bi
\item {\tt domain\_error(comma\_list\_or\_table\_option,Options)}
\ei
\ei
\ei
