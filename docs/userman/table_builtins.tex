...
resolution (\refsec{sec:CompilerOptions}), and according to which
strategy (\refsec{sec:TablingStrategies}).  Further,
...
creation from \refchap{chap:TablingOverview}.  For a detailed
...
some predicate \code{p/n} as ``the table for \code{p/n}.''  Further
...
\index{{\tt use\_subsumptive\_tabling/1}}
%
...
\index{{\tt use\_variant\_tabling/1}}
%
Declares each \emph{tabled predicate} denoted by \code{PredSpec$_i$}
...
\comment{Currently, this builtin fails if CallSkeleton is not a
compound term, but yields an error if either CallSkeleton is not a
tabled predicate or either of TableEntryHandle or ReturnSkeleton are
non-variables.}
...
\comment{Currently, this builtin fails if CallTerm is not a
compound term, but yields an error if either CallTerm is not a
tabled predicate or either of TableEntryHandle or ReturnSkeleton are
non-variables.}
...
\index{{\tt get\_calls\_for\_table/2}}
%
...
\comment{Currently, this builtin fails if \code{ReturnSkeleton} is not
of the form ret(t1,...,tk) or if either k is too small or
\code{ReturnSkeleton} does not unify with any answer.  However, an error
results if (1) \code{TableEntryHandle} is not a valid pointer (memory
violation); (2) \code{ReturnSkeleton} contains too many terms (if k is
too big, then the extra terms are ignored, but this gives the illusion
of a larger than actual substitution); or (3) \code{ReturnHandle} is
already instantiated.}

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

\ournewitem{get\_returns\_for\_call(+CallTerm,?AnswerTerm)}{Tabling}
\index{{\tt get\_returns\_for\_call/2}}
%
...
%
Removes the table entry for the subgoal \code{CallTerm} should it
exists.  The predicate remains tabled and all of its other table
...
\index{{\tt abolish\_table\_pred/1}}
%
