...
substitution to the variables of $S$\@.  The table inspection
predicates allow access to answer substitutions through a family of
terms whose principle functors are \retn, where \code{n} is the number
of distinct variables in the producer subgoal.  The order of arguments
in \retn{} corresponds to the order of distinct variables in a
left-to-right traversal of $S$.

\begin{example}
Let $S =$ \code{p(X,f(Y))} be a producer subgoal (or simply, a subgoal
if call-variance is used) and $\alpha =$ \code{\{X=a,Y=b\}} be an
answer substitution.  The representation of $\alpha$ as a return is
\code{ret(a,b)} and the application of bindings in that return to $S$
yields the answer \code{p(a,f(b))}.\fillBox
\end{example}
...
{\bf Error Cases}
\bi
\item There are currently incomplete tables, or the choice point stack
  contains choice points for backtracking through completed tables.
\bi
\item 	{\tt table\_error}
\ei
%
