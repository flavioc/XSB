...
and/or reclaim their space.  These predicates share similar
characteristics.  In the single-threaded XSB eng¶ine, incomplete
tables may not be abolished by the user, a restriction that is made to
ensure correct evaluations.  Accordingly, calling an {\tt
  abolish\_xxx} predicate when tables are incomplete raises an error.

Second, if the current execution environment contains a choice point
into information that is being abolished, space for the abolished
information is not immediately reclaimed.  More precisely, if the
current execution envonment has a choice point that points to an
answer $A$ we say that $A$ is {\em active in the current environment}.
If a tabled predicate $P$ or subgoal $S$ to be abolished has an answer
that is active in the current environment, reclamation of space for
$P$ or $S$ will be delayed until no answers for $P$ or $S$ are active.
New calls to $P$ or $S$, however, will derive a new table, rather than
using the abolished information.
...
a conditional answer {\tt p(a):- q(b)|}.  If the predicate {\tt q/2)}
(or subgoal {\tt q(b)}) is abolished and later rederived, the
rederivation of {\tt q(b)} may have different semantics than the
original derivation (e.g. if {\tt q(b)} depends on a dynamic predicate
whose definition has changed).  As a separate issue, in \version{}, if
space for {\tt q(b)} is reclaimed, then the call {\tt
  get\_residual(p(a),X)} may core dump.  Accordingly, users of programs
that give rise to conditional answers in completed tables are urged
either to use one of the {\tt abolish\_all\_xxx} predicates, or to
ensure that groups of mutually dependent tables are abolished
together.
...
tables are complete or incomplete, or contain conditional answers.
However, abolishing shared tables can only be done when there is a
single active thread.  In addition, tables private to a thread are
abolished automatically and their space reclaimed when the thread
exits (by normal termination or via an exception).

Finally, we note that any incomplete tables are abolished
...
%
Invalidates all entries from the table for the predicate denoted by
the predicate or term indicator \code{Pred}.  If the table for {\tt
  Pred} contains an answer $A$ that is active in the current
enviornment, {\tt Pred} space reclamation for $A$ will be delayed
until $A$ is no longer active; otherwise the space for {\tt Pred} will
be reclaimed immediately.  If {\tt Pred} contains a conditional
answer, the warning

{\tt abolish\_table\_pred/1 is deleting a table with conditional\ answers: \\
delay dependencies may be corrupted.}

will be issued (cf. the notes at the beginning of this section).  
...
active; otherwise the space for $S$ will be reclaimed immediately.  If
$S$ contains a conditional answer, the warning

{\tt abolish\_table\_call/1 is deleting a table with conditional\ answers: \\ 
delay dependencies may be corrupted.}
...
\code{CallTerm}, should any exist.  With regard to handling subgoals
with answers active in the current environment, incomplete tables, and
multi-threading the predicate acts as {\tt abolish\_table\_pred/1}.
...
including space for conditional answers.  Predicates which have been
declared tabled remain so, but information in their table is deleted.
{\tt abolish\_all\_tables/0} works directly on the memory structures
allocated for table space.  This makes it very fast for abolishing a
large amount of tables, and to maintain its speed it throws an error
if any completed answer $A$ is active in the current execution
environment.  
...
tables for the main thread.  An error will be thrown if there any
completed answer $A$ is active in the current environment, regardless
of whether $A$ is thread-private or thread-shared.
...
\ournewitem{abolish\_all\_private\_tables}{Tabling}
\index{\texttt{abolish\_all\_private\_tables/0}}
%
...
space for conditional answers.  Predicates which have been declared
...
\ournewitem{abolish\_all\_shared\_tables}{Tabling}
\index{\texttt{abolish\_all\_shared\_tables/0}}
%
In the multi-threaded engine, removes all tables private to the thread
and frees all the memory held by XSB for these structures, including
