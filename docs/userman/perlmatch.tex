...
Although the POSIX {\tt Regmatch} package described in the previous section
is more efficient, the Perl interface also provides powerful string
substitution facilities that are not available in the {\tt Regmatch}
package.  The programming interface to the {\tt Perlmatch} package is also
different from that of {\tt Regmatch} --- it closely mimics the interface
