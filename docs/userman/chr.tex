...
informally. They do not essentially differ from other CHR systems.

When a constraint is called in an XSB program, the CHR system will go through
the rules from top to bottom in the order they are written, to see which
rules apply to that constraint and to execute them. Once the constraint is
removed, no more rules are tried. If all the rules have been tried and the
constraint has not been removed, the constraint becomes suspended.

A rule is applicable to a certain constraint, the active constraint, if
other suspended constraints can be found (passive constraints) such that:
\begin{itemize}
\item the active and passive constraints together match the head of the rule
\item the guard of the rule (if any) succeeds without binding any variables in the head
\end{itemize}

\paragraph{Rule Types}
%- - - - - - - - - - 
The semantics of the three types of rules are:
\begin{itemize}
...
\end{verbatim}
where each \texttt{ConstraintSpec} is a functor$\slash$arity pair. Ordinary code
may be freely written between the CHR rules.

The CHR constraints defined in a particulary .chr file are associated
with a module. The module can have any atom value. The default module is
\texttt{user}. A different module name can be declared as follows:
\begin{verbatim}
:- chr_module(modulename).
\end{verbatim}
A program should never consist of different .chr files that have the same
CHR module associated to them.

\subsection{Compilation}

A .chr file containing CHR rules should be run through the CHR pre-processor
to generate a .P file contraining pure XSB code. This .P file can then be
loaded in the XSB emulator and used normally.

\section{Useful Predicates} \label{predicates}
%=========================

The \texttt{chr} module contains several useful predicates that allow you to
inspect and print the content of the constraint store.
...
once and reused many times. This has already shown to be useful for application
...
predicate that has one or more arguments with constraints on them. In the
current release the CHR-related parts of the tabled predicates have to be
written by hand. In a future release this may be substituted by an automatic
transformation.
...
By combining the answer constraits, only the most constrained, or optimal,
answer is kept.
