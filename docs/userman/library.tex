...
\ournewitem{file\_to\_list(IOport, List)}{scrptutl}s
...
{\tt SymConName} must be an atom that denotes an existing
connection, which must be previously created with {\tt socket\_set\_select/4}
(described below). {\tt ReadSockL}, {\tt WriteSockL},
{\tt ErrSockL} are lists of socket handles (as returned by {\tt
...
\ouritem{socket\_set\_select(+SymConName,+ReadSockFdLst,+WriteSockFdLst,+ErrorSockFdLst)} \index{{\tt socket\_select\_destroy/4}}
Creates a connection under the symbolic name {\tt SymConName}
(an atom) for subsequent use by {\tt socket\_select/6}.
{\tt ReadSockFdLst}, {\tt WriteSockFdLst}, and {\tt ErrorSockFdLst} are
lists of sockets for which {\tt socket\_select/6} is to be monitoring read,
...
timeout-enabled: {\tt socket\_connect/4}, {\tt socket\_recv/3},
{\tt socket\_send/3}, {\tt socket\_get0/3}, and {\tt socket\_put/3}.
To set a timeout value for any of the above primitives, the user should
execute {\tt set\_timer/1} right before the subgoal to be timed. 

The most common use of timeouts is to either abort or retry the operation
that timed out. For the latter, XSB provides the {\tt sleep/1} primitive,
