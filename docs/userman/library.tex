...
%% \subsection{Highlevel Interface}
%% 
...
%% 
%% \demo{:- attribute$(Module, (AttributeSpec, \ldots, AttributeSpec$)).}
%% 
%% \noindent where Module is the module name and each $AttributeSpec$ has the
%% form $Functor/Arity$.  Each file can have at most one such declaration.
%% 
%% Having declared some attribute names, these attributes can be added, updated
%% and deleted from unbound variables using the following two predicates
%% (\texttt{get\_atts/3} and \texttt{put\_atts/3}) defined in the module
%% \texttt{atts}. For each declared attribute name in a module, any variable
%% can have at most one such attribute in that module (initially it has none).
%% 
%% \begin{description}
%% \ournewitem{get\_atts(-Var, +Module, ?AccessSpec)}{atts}\index{{\tt get\_atts/3}}
%%     Gets the attributes of \texttt{Var} in module \texttt{Module} according to
...
%%     \end{description}
%% 
%% \ournewitem{put\_atts(-Var, +Module, +AccessSpec)}{atts}\index{{\tt put\_atts/3}}
%%     Sets the attributes of \texttt{Var} in module \texttt{Module} according to
...
\ournewitem{scan(+List,-Tokens)}{stdscan} \index{{\tt scan/2}}  
