...
\section{Communication with Other Processes}
...
\verb|spawn_process/3| comes to the rescue. It allows to spawn any
process (including multiple copies of XSB) and redirect its standard
input and output to XSB streams. XSB can then write to the process and read
from it. The section of socket I/O describes yet another mode of
interprocess communication.

\begin{description}
\ouritem{spawn\_process(+CmdSpec,-StreamToProc,-StreamFromProc)}\index{{\tt spawn\_process/3}} 
Spawn a new process specified by {\tt CmdSpec}. {\tt CmdSpec} must be a
\emph{list} of atoms or Prolog strings (character lists). The first
argument must be the name of the program to run and the other elements are
arguments to the program. Program name must be specified in such a way as
to make sure the OS can find it using the contents of the environment
variable {\tt PATH}.

The remaining parameters to \verb|spawn_process| are XSb I/O streams
...
to the XSB stream {\tt To} the, which is bound to the standard input of the
{\tt cat} process. The process then copies the input to the standard output.
Since standard output of the process is redirected to the XSB stream {\tt
...
output is extremely important here, because XSB I/O streams are buffered.
...
\verb|spawn_process| to not open one of the communication streams.
This is useful when you do not expect to write or read to/from the
subprocess. To tell that a certain stream is not needed, it suffices to
pass a non-variable term to \verb|spawn_process|. For instance, 
%%
\begin{verbatim}
| ?- spawn_process([cat, "-"], To, none),
...
%%
\begin{verbatim}
| ?- spawn_process([cat, "test"], none, From),
...
\end{verbatim}
%%
In each case, only one of the streams is open. Finally, if both streams are
suppressed, then \verb|spawn_process| reduces to the already familiar
{\tt unix/1} call:
%%
\begin{verbatim}
