...
\begin{description}
\ourmoditem{append(?List1, ?List2, ?List3)}{basics}\index{\texttt{append/3}}
%\predindex{append/3~(L)}
    Succeeds if list {\tt List3} is the concatenation of lists 
    {\tt List1} and {\tt List2}.

\ourmoditem{member(?Element, ?List)}{basics}\index{\texttt{member/2}}
...
\ourmoditem{memberchk(?Element, ?List)}{basics}\index{\texttt{memberchk/2}}
...
\ourmoditem{ith(?Index, ?List, ?Element)}{basics}\index{\texttt{ith/3}}
%\predindex{ith/3~(L)}
...
\ourmoditem{delete\_ith(+Index, +List, ?Element, ?RestList)}{listutil}
...
\ourmoditem{log\_ith(?Index, ?Tree, ?Element)}{basics}\index{\texttt{log\_ith/3}}
...
\ourmoditem{log\_ith\_bound(?Index, ?Tree, ?Element)}{basics}
...
\ourmoditem{length(?List, ?Length)}{basics}\index{\texttt{length/2}}
...
\ourmoditem{same\_length(?List1, ?List2)}{basics}\index{\texttt{same\_length/2}}
...
\ourmoditem{select(?Element, ?L1, ?L2)}{basics}\index{\texttt{select/3}}
%\predindex{select/3~(L)}
    {\tt List2} derives from {\tt List1} by selecting (removing) an 
    {\tt Element} non-deterministically.

\ourmoditem{reverse(+List, ?ReversedList)}{basics}\index{\texttt{reverse/2}}
...
\ourmoditem{perm(+List, ?Perm)}{basics}\index{\texttt{perm/2}}
%\predindex{perm/2~(L)}
...
\ourmoditem{subseq(?Sequence, ?SubSequence, ?Complement)}{basics}
...
\ourmoditem{merge(+List1, +List2, ?List3)}{listutil}\index{\texttt{merge/3}}
%\predindex{merge/3~(L)}
...
\ourmoditem{absmerge(+List1, +List2, ?List3)}{listutil}\index{\texttt{absmerge/3}}
    Predicate {\tt absmerge/3} is similar to {\tt merge/3}, except that 
    it uses predicate {\tt absmember/2} described below rather than 
    {\tt member/2}.

\ourmoditem{absmember(+Element, +List)}{listutil}\index{\texttt{absmember/2}}
...
\ourmoditem{member2(?Element, ?List)}{listutil}\index{\texttt{member2/2}}
...
\ourmoditem{closetail(?List)}{listutil}\index{\texttt{closetail/1}}
...
\ourmoditem{comma\_to\_list(+CommaList,-List)}{basics}
...
%
\ourmoditem{comma\_member(?Element,?CommaList)}{basics}
...
\begin{description}
\ourmoditem{get\_attr(-Var,+Mod, ?Val)}{machine}\index{\texttt{get\_attr/3}} 
%
...
\ourmoditem{put\_attr(-Var,+Mod, ?Val)}{machine}\index{\texttt{put\_attr/3}}
%
...
\ourmoditem{del\_attr(-Var,  +Mod)}{machine}\index{\texttt{del\_attr/2}} 
%
...
\ourmoditem{install\_attribute\_portray\_hook(Module,Attribute,Handler)}{machine}
\index{install\_attribute\_portray\_hook/3}
%
...
\ourmoditem{install\_attribute\_constraint\_hook(Module,Vars,Names,Handler)}{machine}
\index{install\_attribute\_constraint\_hook/4}
%
...
\begin{description}
\ourmoditem{when(+Condition,Goal)}{constraintLib}\index{\texttt{when/2}}
%
...
\ourmoditem{setarg(+Index,+Term,+Value)}{constraintLib}\index{\texttt{setarg/3}}
%
...
\ourmoditem{term\_variables(+Term,-Variables}{constraintLib}
\index{\texttt{term\_variables/2}}
% 
...
\begin{description}
\ourmoditem{format(+String,+Control)}{format}\index{\texttt{format/2}}
\ourmoditem{format(+Stream,+String,+Control)}{format}\index{\texttt{format/3}}
...
%\ourmoditem{current\_input\_port(-IOport)}{curr\_sym} \index{\texttt{current\_input\_port/}}
%   See {\tt current\_output\_port/1}.
%\ourmoditem{current\_output\_port(-IOport)}{curr\_sym} \index{\texttt{current\_output\_port/}}
...
%\ourmoditem{file\_open(+FileName,+Mode,-IOport)}{file\_io} \index{\texttt{file\_open/3}}
...
%
%\ourmoditem{file\_close(+IOport)}{file\_io}\index{\texttt{file\_close/1}}
%    Closes the file (or string) for descriptor {\tt IOport}.

%\ourmoditem{ioport2iostream(+IOport, -Stream)}{file\_io}
...
\ourmoditem{file\_write(+IOport,+Term)}{xsb\_writ}
\index{\texttt{file\_write/2}}
    Writes the term {\tt Term} to the file (or string) with descriptor {\tt
    IOport}.

\ourmoditem{file\_read(+IOport,-Term)}{xsb\_read}
...
\ourmoditem{file\_read(+IOport,-Term,-Vars)}{xsb\_read}
...
\ourmoditem{file\_read\_canonical(+IOport,-Term,-Psc)}{machine}
...
\ourmoditem{fget\_line(+Str,-Inlist,-Next)}{scrptutl}
...
\begin{description}
\ourmoditem{str\_sub(+Sub, +Str, ?Pos)}{string}
...
\ourmoditem{str\_match(+Sub, +Str, +Direction, ?Beg, ?End)}{string}
...
\comment{
% taken out -- most people should be using atom_length/2
\ourmoditem{str\_length(+Str, ?Result)}{string}
...
%%
\ourmoditem{substring(+String, +BeginOffset, +EndOffset, -Result)}{string}
...
\ourmoditem{string\_substitute(+InpStr, +SubstrList, +SubstitutionList, -OutStr)}{string}
...
\ourmoditem{concat\_atom(+AtomList,?Atom)}{string}
...
\ourmoditem{concat\_atom(+AtomList,+Sep,?Atom)}{string}
...
\begin{description}
\ourmoditem{expand\_filename(+FileName,-ExpandedName)}{machine}
...
\ourmoditem{expand\_filename\_no\_prepend(+FileName,-ExpandedName)}{shell}
...
\ourmoditem{parse\_filename(+FileName,-Dir,-Base,-Extension)}{machine}
...
\ourmoditem{sleep(+Seconds)}{shell}
\index{\texttt{sleep/1}}
...
\ourmoditem{sys\_pid(-Pid)}{shell}
\index{\texttt{pid/1}}
Get Id of the current process.

\ourmoditem{getenv(+VarName,-VarVal}{machine} 
\index{\texttt{getenv/2}}
%
...
\ourmoditem{putenv(+String)}{machine} 
\index{\texttt{putenv/3}}
%
...
\ourmoditem{file\_time(+FileName, -time(Time1,Time2))}{file\_io}
...
\ourmoditem{file\_size(+FileName, -Size)}{file\_io}
\index{\texttt{file\_size/2}}

Returns file size.

\ourmoditem{cwd(-Dir)}{shell}
\index{\texttt{cwd/1}}
Get current working directory.

\ourmoditem{is\_absolute\_filename(+FileName)}{machine} 
...
\ourmoditem{cd(+Path)}{shell}
\index{\texttt{cd/1}}
Change directory.

\ourmoditem{rename(+Old,-New)}{shell}
\index{\texttt{rename/2}}
Rename file.

\ourmoditem{directory(+Path,?Directory)}{directry}
...
\ourmoditem{file\_to\_list(S, List)}{scrptutls}
...
%% Do not use it!
%%
%%\ourmoditem{sysin(+Command,?Output)}{scrptutl}
%%\index{\texttt{sysin/2}}
%%
...
%
\ourmoditem{get\_process\_table(-ProcessList)}{shell}
...
%%
\begin{description}
\ourmoditem{fd2ioport(+Pipe, -IOport)}{shell}\index{\texttt{fd2ioport/2}} 
...
\begin{description}
\ourmoditem{array\_new(-Array,+Size)}{array}\index{\texttt{array\_new/2}}
%
Creates a one dimensional empty array of size {\tt Size}.  All the
elements of this array are variables.  

\ourmoditem{array\_elt(+Array, +Index, ?Element)}{array} \index{\texttt{array\_elt/3}} 
%
Succeeds iff {\tt Element} unifies with the {\tt Index}-th element of
array {\tt Array}.  
%
\ourmoditem{array\_update(+Array, +Index, +Elem)}{array} \index{\texttt{array\_update/3}} 
%
...
\begin{description}
\ourmoditem{profile\_call(+Goal)}{xsb\_profiling}\index{profile\_call/1}
...
\begin{description}
\ourmoditem{profile\_mode\_call(+Goal)}{xsb\_profiling}\index{profile\_mode\_call/1}
...
\begin{description}
\ourmoditem{profile\_mode\_dump}{xsb\_profiling}\index{profile\_mode\_dump/0}
...
\begin{description}
\ourmoditem{profile\_mode\_init}{xsb\_profiling}\index{profile\_mode\_init/0}
...
\begin{description}
  %%
\ourmoditem{new\_trie(-Root)}{intern} \index{\texttt{newtrie/1}}
        {\tt Root} is instantiated to a handle for a new trie.

\ourmoditem{trie\_intern(+Term,+Root)}{intern}
...
\ourmoditem{trie\_interned(?Term,+Root)}{intern}
...
\ourmoditem{delete\_trie(+Root)}{intern} \index{\texttt{delete\_trie/1}}
%%
...
\begin{description}
  %%
\ourmoditem{trie\_intern(+Term,+Root,-Leaf,-Flag,-Skel)}{intern}
...
\ourmoditem{trie\_intern(+Term,-Leaf,-Skel)}{intern}\index{\texttt{trie\_intern/3}}
%%
Acts as {\tt trie\_intern/5}, but interns {\tt Term} into the default
trie and does not return the new/old flag.

\ourmoditem{trie\_interned(?Term,+Root,?Leaf,-Skel)}{intern}
...
\ourmoditem{trie\_interned(?Term,?Leaf,-Skel)}{intern} \index{\texttt{trie\_interned/3}}
%%
Similar to {\tt trie\_interned/4}  but uses the default trie.

\ourmoditem{trie\_unintern(+Root,+Leaf)}{intern} \index{\texttt{trie\_unintern/2}}
%%
...
\ourmoditem{trie\_unintern\_nr(+Root,+Leaf)}{intern}\index{\texttt{trie\_unintern\_nr/2}}
%%
...
\ourmoditem{unmark\_uninterned\_nr(+Root,+Leaf)}{intern}\index{\texttt{unmark\_uninterned\_nr/2}}
...
\ourmoditem{reclaim\_uninterned\_nr(+Root)}{intern}
...
\begin{description}
\ourmoditem{prepare(+Index)}{gensym} \index{\texttt{prepare/1}}
%
...
\ourmoditem{gennum(-Var)}{gensym} \index{\texttt{gennum/1}}
%
Unifies {\tt Var} with a new integer.

\ourmoditem{gensym(+Atom,-Var)}{gensym} \index{\texttt{gensym/2}}
%
...
\ourmoditem{random(-Number)}{random} \index{\texttt{random/1}}
%
Binds \texttt{Number} to a random float in the interval [0.0, 1.0).
Note that 1.0 will never be generated.

\ourmoditem{random(+Lower,+Upper,-Number)}{random} \index{\texttt{random/3}}
...
\ourmoditem{getrand(?State)}{random} \index{\texttt{getrand/1}}
...
\ourmoditem{setrand(rand(+X,+Y,+Z))}{random} \index{\texttt{setrand/1}}
...
\ourmoditem{datime\_setrand}{random} \index{\texttt{datime\_setrand/0}}
...
\ourmoditem{randseq(+K, +N, -RandomSeq)}{random} \index{\texttt{randseq/3}}
...
\ourmoditem{randset(+K, +N, -RandomSet)}{random} \index{\texttt{randset/3}}
...
\ourmoditem{gauss(-G1,-G2)}{ramdom} \index{\texttt{gauss/2}}
%
...
\ourmoditem{weibull(K,Lambda,X)}{ramdom} \index{\texttt{weibull/3}}
%
...
\ourmoditem{exponential(K,X)}{ramdom} \index{\texttt{exponential/2}}
%
...
\ourmoditem{scan(+List,-Tokens)}{stdscan} \index{\texttt{scan/2}}  
...
\ourmoditem{scan(+List,+FieldSeparator,-Tokens)}{stdscan} 
\index{\texttt{scan/3}}  
...
\ourmoditem{set\_scan\_pars(+List)}{stdscan} 
...
\ourmoditem{get\_scan\_pars(-List)}{stdscan} 
...
\ourmoditem{term\_hash(+Term,+HashSize,-HashVal)}{machine}
...
\ourmoditem{pretty\_print(+ClausePairs)}{pretty\_print}
...
\subsection{Ordered Sets: {\tt ordsets.P}}
...
%
\comment{ The cost of the free list is slight.  The storage cost of a
  heap in a copying Prolog (which Dec-10 Prolog is not) is 2K+3M
  words.}
