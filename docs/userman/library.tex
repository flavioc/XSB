...
I/O, the low-level routines use \emph{XSB file descriptors} (or I/O ports)
to refer to files. XSB file descriptors should not be confused with the
file descriptors used by the OS ({\it e.g.}, the small integers returned by
the C {\tt open} function). XSB file descriptors are indices into the XSB
table of open files. Typically XSB opens files for buffered I/O (whether
using the stream I/O predicates or the predicates described here), so XSB
file descriptors actually correspond to {\tt FILE} data structures like
those returned by the C {\tt fopen} function.

When it starts, XSB opens a number of standard file descriptors that it uses to
...
\ournewitem{current\_input\_fd(-FileDes)}{curr\_sym} \index{{\tt current\_input\_fd/}}
   See {\tt current\_output\_fd/1}.
\ournewitem{current\_output\_fd(-FileDes)}{curr\_sym} \index{{\tt current\_output\_fd/}}
   The above two predicates instantiate {\tt FileDes} to the XSB file
   descriptor for the current user input and output ({\it i.e.}, the things
   that are manipulated through {\tt see/seen} and {\tt tell/told} predicates).
   Once the file descriptor is obtained, it is possible to safely use the
   lower-level I/O predicates described below interchangeably with stream
   I/O. 
    
\ournewitem{file\_open(+FileName,+Mode,-FileDes)}{file\_io} \index{{\tt file\_open/3}}
    Opens a file with name {\tt FileName} to be accessed
    in mode {\tt Mode} and returns a file-descriptor in {\tt FileDes} that
...
\ournewitem{file\_reopen(+FileName,+Mode,+FileDes,-RetCode)}{file\_io}\index{{\tt file\_reopen/1}}
    Takes an existing file descriptor, closes it, then opens it and
...
\ournewitem{file\_clone(+SrcFileDes,?DestFileDes,-RetCode)}{file\_io}\index{{\tt file\__clone/1}}
    This is yet another way to redirect I/O. It is a prolog interface to
    the C {\tt dup} and {\tt dup2} system calls. If {\tt DestFileDes} is a
    variable, then this call creates a new XSB file descriptor that is a
    clone of {\tt SrcFileDes}. This means that I/O sent to either
    descriptor goes to the same place. If {\tt DestFileDes} is not a
    variable, then it must be a number corresponding to a valid file
    descriptor. In this case, XSB closes {\tt DestFileDes} and makes it
    into a clone on {\tt SrcFileDes}. For instance, suppose that 10 is a
    file descriptor that is currently open for writing to file {\tt foo.bar}.
    Then
    %%
    \begin{verbatim}
    | ?- file_clone(10,3,_).      
    ...
    \end{verbatim}
    %%
    This means that file descriptor 10 now becomes clone of standard
...
\ournewitem{file\_pos(+FileDes, -Position)}{file\_io}
\index{{\tt file\_pos/2}}
...
\ournewitem{file\_getbuf(+FileDes, +BytesRequested, -String, -BytesRead)}{file\_io}
...
\ournewitem{file\_getbuf\_atom(+FileDes, +BytesRequested, -String, -BytesRead)}{file\_io}
...
\ournewitem{file\_getbuf\_list(+FileDes, +BytesRequested, -CharList, -BytesRead)}{file\_io}
...
\ournewitem{file\_putbuf(+FileDes, +BytesRequested, +String, +Offset, -BytesWritten)}{file\_io}
...
{\tt pipe2file/2}, must be imported from module {\tt shell}.
The predicates {\tt pipe\_open/2} and
{\tt pipe2file/2} must be imported from {\tt file\_io}.
...
\ouritem{pipe\_open(-ReadPipe, -WritePipe)}\index{{\tt pipe\_open/2}} 
  ...
  that represent file handles of the underlying OS. They {\bf are not XSB file
  descriptors}, and they cannot be used for I/O directly. To use them, one
  must call the {\tt pipe2file/2} predicate, as described next.\footnote{
    %%
    XSB does not convert pipes into file descriptors automatically because.
    Because of the way XSB file descriptors are represented, they are not
    ...
    must pass the child processes an OS file handle instead. The child then
    converts these handles into XSB file descriptors.
    %%
    }
\ouritem{pipe2file(+Pipe, -FileDes)}\index{{\tt pipe2file/2}} 
    Take a pipe and convert it to an XSB file descriptor that can be used
    for I/O.
  ...
  After that, the child reads a message from the pipe and prints it to its
    ...
    \begin{verbatim}
%% parent.p      
:- import pipe_open/2, pipe2file/2, fmt_write/3, file_flush/2 from file_io.
...
yes       
    ...
    Finally, we should note htat this mechanism can be used to communicate
    through pipes with
    non-XSB processes as well. Indeed, an XSB process can create
    a pipe using {\tt pipe\_open}, pass one end of the pipe to a child
    process (which can be a C program), and use {\tt pipe2file} to convert
    the other end of the pipe to an XSB file. The C program, of course,
    does not need {\tt pipe2file}, since it can use the pipe file handle
    directly. Likewise, a C program can spawn off an XSB process and pass
    it one end of a pipe. The XSB child-process can then convert this pipe
    to a file using {\tt pipe2file} and then talk to the paren C program.
