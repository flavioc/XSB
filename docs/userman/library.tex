...
\verb|spawn_process/5| comes to the rescue. It allows to spawn any
...
The next three parameters of \verb|spawn_process| are XSB I/O streams
to the process (pipe to the subprocess standard input), from the process
(pipe from the subprocess standard output), and a stream capturing the
...
\verb|spawn_process/5| to not open one of the communication streams.
...
pass a non-variable term to \verb|spawn_process/5|. For instance, 
%%
\begin{verbatim}
...
\ouritem{get\_process\_table(-ProcessList)}\index{{\tt get\_process\_table/1}} 
    ...
    Each term in the pist is of the form:
