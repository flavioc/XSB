...
%\predindex{load\_dyn/1~(L)}
    ...
    To do this, put the folowing in the source file: 
    %%
    \begin{verbatim}
...
at both the term level and the character level.  Unlike the standard Prolog
stream I/O, the low-level routines use \emph{XSB file descriptors} (or I/O
ports) to refer to files. XSB file descriptors should not be confused with
the file descriptors used by the OS ({\it e.g.}, the numberd returned by
...
\ournewitem{file\_open(+FileName,+Mode,-FileDes)}{file\_io} \index{{\tt file\_open/3}}
    ...
    can be used to access the file.  If {\tt Mode} is 0 ({\tt OREAD}), the
    the file is opened for reading; if it is 1 ({\tt OWRITE}), the file is
    opened for writing; if it is 2 ({\tt OAPPEND}), the file is opened for
    appending.  If {\tt Mode} is 3 ({\tt OAPPEND}), then the string making
...
%%
\begin{verbatim}
    | ?- file_reopen('/dev/null', 1, 3, Error).
\end{verbatim}
%%
...
All predicates in this section must be imported from module {\tt shell}.
...
shell/5.
\ouritem{sys\_exit(-ExitCode)}\index{{\tt sys\_exit/5}} 
...
(dsw - This is stuff Hasan wrote, wich ought to be formated, cleaned up and included: )
