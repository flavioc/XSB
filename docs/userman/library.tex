...
%--------------------------------------------------------------------------------------------
\comment{ TLS: I think I put this in, and I think its overtaken by
...
input and write to standard output. The next line writes a
newline-terminated string to the XSB stream {\tt To} the, which is
bound to the standard input of the {\tt cat} process. The process then
copies the input to the standard output.  Since standard output of the
process is redirected to the XSB stream {\tt From}, the last line in
our program is able to read it and return in the variable {\tt Y}.

Note that in the second line we used {\tt file\_flush/2}. Flushing the
output is extremely important here, because XSB I/O pipe (file)
streams are buffered.  Thus, {\tt cat} might not see its input until
the buffer is filled up, so the above clause might hang. {\tt
file\_flush/2} makes sure that the input is immediately available to
the subprocess.

In addition to the above general schema, the user can tell
\verb|spawn_process/5| to not open one of the communication streams or to
...
\item It is very important to not forget to close the streams that the
  ...
  standard input of the child.  forts will remain open, since they
  belong to the parent process. As a result, the parent will own
  defunct I/O streams and might eventually run out of file
  descriptors.
\end{enumerate}
...
\ouritem{get\_process\_table(-ProcessList)}\index{\texttt{get\_process\_table/1}} 
...
\ouritem{pipe\_open(-ReadPipe, -WritePipe)}\index{\texttt{pipe\_open/2}} 
  ...
  use them, one must call the {\tt fd2stream/2} predicate, as described
  next.\footnote{
    %%
...
Take a pipe and convert it to an XSB I/O stream. This predicate is
needed because pipes must be associated with XSB I/O streams before
any I/O can be done on them by an XSB program.
  ...
  After that, the child reads a message from the pipe and prints it to its
    ...
    \begin{verbatim}
%% parent.P      
:- import pipe_open/2, fd2iostream/2, fmt_write/3, file_flush/2 from file_io.
...
?- pipe_open(RP,WP),
   ...
   file_close(ProcInput), file_close(WF).
    ...
    \begin{verbatim}
%% child.P
:- import fd2iostream/2 from file_io.
:- import file_read_line_atom/2 from file_io.
:- dynamic pipe/1.
?- pipe(P), fd2iostream(P,F),
...
yes       
    ...
    fd2iostream} to convert the other end of the pipe to an XSB
    stream. The C program, of course, does not need {\tt fd2ioport},
    ...
    file using {\tt fd2ioport} and then talk to the parent C program.
