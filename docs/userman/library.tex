...
In XSB attributed variabels now comes with a new interface.  It  allows the
programmer more control over structuring attributes than in SICStus.
...
needs to be {\tt dynamic} (so that it is retractable), when the code
size is too large for the XSB compiler (if a file has hundreds of
thousands or more clauses), or when it contains atoms whose length is
more than 255 that cannot be handled by the XSB compiler.
...
%\predindex{load\_dyn/1~(L)}
    ...
    \begin{description}
    \item[{\tt type\_error}]
	{\tt FileName} is not an atom at the time of call.
    \end{description}

\ournewitem{load\_dyn(+FileName,+Dir)}{consult}
\index{\texttt{load\_dyn/2}}\label{load_dyn/2}
    Asserts the contents of file {\tt FileName} into the database.  {\tt Dir}
    indicates whether {\tt assertz} or {\tt asserta} is to be used.  If {\tt Dir} 
    is 1, then {\tt assertz} is used and the behavior of {\tt load\_dyn(FileName)}
    is obtained.  If {\tt Dir} is 0, then {\tt asserta} is used to add the 
    clauses to the database, and clauses will be in the reverse order of
    their appearance in the input file.  {\tt asserta} is faster than {\tt assertz}
    for predicates such that their indexing and data result in many hash collisions.

\ournewitem{ensure\_dyn\_loaded(+FileName)}{consult}
\index{\texttt{ensure\_dyn\_loaded/1}}
...
\ournewitem{load\_dync(+FileName)}{consult}\index{\texttt{load\_dync/1}}
\index{canonical forma}
    ...
    speed is important.  (See {\tt load\_dync/2 below for further efficiency 
    considerations.}  A file that is to be dynamically loaded often but
...
\index{\texttt{load\_dync/2}}\label{load_dync/2}
    ...
    ``canonical'' format.  {\tt Dir} indicates whether {\tt assertz} 
    or {\tt asserta} is to be used.  If {\tt Dir} is 1, then {\tt assertz}
    is used and the exact behavior of {\tt load\_dync(FileName)}
    is obtained.  If {\tt Dir} is 0, then {\tt asserta} is used to add the 
    clauses to the database, and clauses will end up in the reverse order of
    their appearance in the input file.  Using {\tt Dir} of 0 can sometimes
    be {\em much} faster than the default of 1.  The reason has to do with how
    indexes on dynamic code are represented.  Indexes use hash tables with bucket
    chains.  No pointers are kept to the ends of bucket chains, so when adding
    a new clause to the end of a bucket (as in {\tt assertz}), the entire chain
    must be run.  Notice that in the limiting case of only one populated bucket 
    (e.g., when all clauses have the same index term), this makes assertz-ing a
    sequence of clauses quadratic.  However, when using {\tt asserta}, the new
    clause is added to the beginning of its hash bucket, and this can be done in
    constant time, resulting in linear behavior for asserta-ing a sequence of
    clauses.

{\tt asserta} is faster than {\tt assertz}
    for predicates such that their indexing and data result in many hash collisions.

\ournewitem{ensure\_dync\_loaded(+FileName)}{consult}
\index{\texttt{ensure\_dync\_loaded/1}}
...
{\tt regmatch} package described in Chapter~\ref{chap-posix}.
...
%-----------------------------------------------------------------------
\comment{
TLS: use concat_atom, or atom_concat. 
\ournewitem{str\_cat(+Str1, +Str2, ?Result)}{string}
\index{\texttt{str\_cat/3}}
...
placing the result in {\tt Term}.  This predicate is for convenience,
as any atom can be opened and read using XSB's streams.  The accepted
syntax is intended to be valid canonical form (with no trailing '.'),
extended by a treatment of the usual list syntax.  It should be the
inverse of {\tt term\_to\_atom/2}.  Floating point numbers are not
completely handled; only a fixed point representation is used.  If the
atom is not a syntactically valid term, the predicate fails, quietly.

\ournewitem{codes\_to\_term(+CodeList,-Term)}{string}
\index{\texttt{codes\_to\_term/2 }}
...
\index{\texttt{read\_atom\_to\_term/2 }}
...
it represents.  This predicate is for convenience, as any atom can be
opened and read using XSB's streams.  A minor difference with the
stream-based approach is that the atom should {\em not} contain a
terminating period ('.'). If the atom is not a syntactically correct
term, then this predicate fails, quietly.

\ournewitem{read\_atom\_to\_term(+Atom,-Term,-VarList)}{string}
\index{\texttt{read\_atom\_to\_term/3 }}

This predicate converts an atom {\tt Atom} whose characters make up a
valid term that can be read by {\tt read/1} into the term ({\tt Term})
it represents.  This predicate is for convenience, as any atom can be
opened and read using XSB's streams and {\tt read\_term/3}.  A minor
difference with the stream-based approach is that the atom should {\em
not} contain a terminating period ('.'). If the atom is not a
syntactically correct term, then this predicate fails, quietly.
...
Prolog, (or XSB) can be useful for writing scripts.  Prolog's simple
syntax and declarative semantics make it especially suitable for
scripts that involve text processing.  There are several ways to
access script-writing commands from XSB.  The first is to execute the
command via the predicates {\tt shell/1} or {\tt shell/2}.  These
predicates can execute any command but they do not provide streamablity
across UNIX and Windows commands, and they do not return any output of
commands to Prolog.  Special predicates are provided to handle
cross-platform compatability and to bring output into XSB.  
...
\begin{description}
  %%
\ournewitem{newtrie(-Root)}{intern} \index{\texttt{newtrie/1}}
        Root is instantiated to a handle for a new trie.

\ournewitem{trie\_intern(+Term,+Root,-Leaf,-Flag,-Skel)}{intern}\index{\texttt{trie\_intern/5}}
%%
...
\ournewitem{trie\_interned(?Term,+Root,?Leaf,-Skel)}{intern}\index{\texttt{trie\_interned/4}}
%%
...
\ournewitem{genrand(?State)}{random} \index{\texttt{genrand/1}}
...
The xsb\_lint\_impexp.P file contains a simple tool to analyze
...
and generating document\_import/export declarations that might be
...
the warning messages to be issued, if appropriate.  This allows a user
...
To use this tool, [xsb\_lint\_impexp] must be consulted, which will
define the checkImpExps/1/2 and add\_libraries/1 predicates.

\begin{description}
\ournewitem{add\_libraries(+DirectoryNameList)}{xsb\_lint\_impexp}
...
\begin{description}
\ournewitem{checkImpExps(+Options,+FileNameList)}{xsb\_lint\_impexp}
...
\begin{enumerate}
\item
   used\_elsewhere: Print a warning message in the case of a predicate
   defined in a file, not used there, but used elsewhere.  This can be
   useful to see whether it might be better to move the predicate
   definition to another file, but it produces many warnings for
   predicates multi-use libraries libraries.
\item
   unused: Print a warning message in the case of a predicate that is
...
\item
   all\_files: By default, only predicates in files that contain a {\tt
   :- document\_export} or {\tt :- export} declaration are processed.
   This option causes predicates of {\em all} files (and modules) to be
   processed.
\item
   all\_symbol\_uses: Treat {\em all} non-predicate uses of symbols
   (even constants) as predicate uses for the purpose of generating
   imports.
\item
   no\_symbol\_uses: Don't treat any non-predicate uses of symbols as
...
\begin{description}
\ournewitem{checkImpExps(++FileNameList)}{xsb\_lint\_impexp}

{\tt checkImpExps/1} calls {\tt checkImpExps/2} with the default
options.
\end{description}
