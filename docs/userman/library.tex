...
\ouritem{remove\_dupls(+List1, ?List2)}\index{{\tt remove\_dupls/2}}
    There is no need to provide such a predicate.  If you need one use
    the {\tt sort/2} standard predicate (described in section~\ref{Comparison}).
\end{description}
...
\ourprolog's database.  The use of these predicates may be necessary
...
\ournewitem{fmt\_read(+Fmt,+Types,-Term,-Ret)}{machine}\index{{\tt fmt\_read/4}}
...
\ournewitem{fmt\_write(+Fmt,+Term)}{machine}\index{{\tt fmt\_write/2}}
%\predindex{fmt\_write/2~(L)}
...
\ournewitem{fmt\_write\_string(-String,+Fmt,+Term)}{machine}\index{{\tt fmt\_write\_string/2}}
...
\ournewitem{read\_canonical(-Term)}{basics}\index{{\tt read\_canonical/1}}
    Reads a term that is in canonical format from the current input
    stream and returns it in {\tt Term}. On end-of-file, it returns the
    atom {\tt end\_of\_file}.  If it encounters an error, it prints an
    error message on stderr and returns the atom {\tt
...
\ournewitem{file\_open(+FileName,+Mode,-FileDes)}{machine}
\index{{\tt file\_open/3}}
    ...
    \ourprolog 's term I/O routines to build terms from atoms.

\ournewitem{file\_close(+FileDes)}{machine}\index{{\tt file\_close/1}}
...
\ournewitem{file\_time(+FileName, -time(Time1,Time2))}{machine}
...
\ournewitem{file\_read\_line(+FileDes,-String,-IsFullLine)}{machine}
...
Under NT and DOS, this predicates does rectification (using backslashes
when appropriate), but it does not expand the tildas.
...
\index{{\tt is\_absolute\_filename/1}}
Succeeds, if file name is absolute; fails otherwise.
This predicate works also under NT and DOS, {\it i.e.}, it recognizes drive
letters, etc.
...
name starts with a tilde (in Unix), then it is expanded.
...
recommended programming practice is as outlined in Section
\ref{sec:assert}.  For compatability with previous versions, the
...
specification of these predicates is {\tt trie} (see Section
\ref{sec:assert}), the predicates are defined as {\tt assert/1}, {\tt
...
As explained in Section \ref{tabling_overview} XSB can compute normal
logic programs according to the well-founded semantics.  In fact, XSB
