...
%\predindex{fmt\_read/4~(L)}
    ...
    file\_io}.  {\tt Fmt} must be a string (list of characters) or an atom
    ...
    Note that the C escape characters are NOT recognized.  So, for example,
    to include a 
    newline, one must put a newline directly in the atom, and not use
    \verb|\n|.
    Another difference is that, unlike most C compilers, XSB insists that a
    single {\tt \%} in the format string signifies format conversion
...
\ournewitem{fmt\_write(+Fmt,+Term)}{file\_io}\index{{\tt fmt\_write/2}}
    ...
    {\tt Fmt} must be a string or an atom representing the format that will be
    passed to the C call to {\tt printf}.  See the C documentation
    for {\tt printf} for the meaning of this string.  Note that
    the C escape characters are NOT
...
\ournewitem{fmt\_write\_string(-String,+Fmt,+Term)}{file\_io}\index{{\tt fmt\_write\_string/2}}
    ...
    string or an atom that represents the format.
