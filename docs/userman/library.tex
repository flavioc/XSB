...
\ournewitem{file\_reopen(+FileName,+Mode,+IOport,-RetCode)}{file\_io}\index{\texttt{file\_reopen/1}}
...
\ournewitem{tmpfile\_open(-IOport)}{file\_io}\index{\texttt{tmpfile\_open/1}}
    Opens a temporary file with a unique filename. The file is deleted when
    it is closed or when the program terminates.

\ournewitem{file\_clone(+SrcIOport,?DestIOport,-RetCode)}{file\_io}\index{\texttt{file\_clone/1}}
...
predicates can execute any command but they do not provide portablity
