...
subsumptive tabling, the total number of subgoal calls is given, as is
the number of new calls ({\tt producers}) and the number of repeated
calls to non-completed tables ({\tt variants}).  Furthermore, the
number of properly subsumed calls is given, along with the number of
table entries constructed for these subsumed calls.  Finally, the
number of subsuming calls to completed tables is given.
%
...
Tabling Operations
  Call Subsumption Subgoal Operations:
            call check/insert ops: 0 producers, 0 variants,
            0 properly subsumed (0 table entries), 
            0 used completed table.
