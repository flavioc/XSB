...
The first set of functions is typically be used to check the type of the
...
the next task is to convert this data into the types that C understands.
This is done with the folowing functions. The first three convert between
the basic types. The last two extract the functor name and the arity.
Extraction of the components of a list and the arguments of a structured
...
\ouritem{prolog\_term p2p\_unify(prolog\_term T1, prolog\_term T2)}
    \index{{\tt p2p\_unify}}
