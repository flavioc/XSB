...
The high level foreign predicate interface was designed to release,
from the programmer, the burden of having to write specific code to
transfer arguments to \ourprolog\ to C and vice-versa.

By introducing a new and improved notation for describing C functions
and its counterpart Prolog predicates in the .H files, this module is
capable of automatically generating code (called \emph{wrappers})
which will be responsible for translating Prolog terms and structures
to proper C types, and vice-versa. The \emph{wrappers} are
automatically used when compiling the foreign predicates\footnote{In
  Windows, this is not true. Due to the complexity of creating
  Makefiles for the different Windows compilers, it's not yet
  implemented, and the user is responsible for compiling the foreign
  module along with the created wrappers}.
...
{\tt :- foreign\_pred \emph{predname}(\emph{[+-]parg1,
  [+-]parg2,...}) from \emph{funcname}(\emph{carg1:type1, carg2:type2,
  ...}):\emph{functype}.
}
\end{center}
...
preceded by either '+' or '-', indicating its mode as, respectively,
input or output.The names of the arguments are the same as the
ones declared for the C function. Note that a C function argument may
appear as at most two positions in the predicate argument list, as
long as both have different modes. Also, the identifier
\texttt{retval} is used to mean the return value of the C function,
and if used has always to have mode '-'.
...
function. The names used for the arguments must match with the names
used in the Prolog declaration.
...
descriptive types, defined below.

\ouritem{functype} is the return type of the C function.

\end{description}

This notation allows the use of pre-defined C functions, which were
not designed to be a prolog foreign predicate. Also, it's much useful
when designing new foreign predicates, once the user can concentrate
on the actual functionality of the C code, and not on details specific
to how to transfer data to and from Prolog.

Table~\ref{table:hltypes} shows which types are allowed in the C
function declaration, and to which actual C types they relate. Note
that the associate C type of a descriptive type may change according
to the usage of the argument in the Prolog predicate definition.
...
As stated before, the system doesn't attempt to compile and build the
DLL for the foreign module on Windows, due to the complexity of
creating makefiles for the different compilers.

In order to use this facility, the user has to create the DLL herself.
The process is, roughly, as follows: first, she has to compile the
module from within \ourprolog\. This will create the \ourprolog\ 
specific object file, and the \emph{wrappers}. The \emph{wrappers} are
created in a file named
\texttt{xsb\_wrap\_}\emph{modulename}\texttt{.c}.

Then, the user has to create a project, in her preferred compiler,
for a dynamically-linked library. In this project, she will include
the module source code, along with the \emph{wrapper} created by
XSB. Also, this DLL should be linked against the library
\texttt{XSB/config/x86-pc-windows/bin/xsb.lib}, which is provided with
XSB.
