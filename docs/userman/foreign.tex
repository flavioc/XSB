...
\desc{float ptoc\_float(int N)}
        ...
        depending on whether or not precise floats are enabled. This can be done
        by specifying the --enable-precise-floats flag at configure time, which 
        would turn all normal prolog floats into double values, stored on the heap).
