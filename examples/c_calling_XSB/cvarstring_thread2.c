...
void *command_ps_fail(void * arg) {
  ...
  if ((rc = xsb_command_string(p_th,"test_p_fail.")) == XSB_ERROR)
    fprintf(stderr,"Command error p: %s/%s\n",xsb_get_error_type(p_th),xsb_get_error_message(p_th));
  else if ((rc = xsb_command_string(p_th,"test_p_fail.")) == XSB_FAILURE)
...
void *command_rs_err(void * arg) {
  ...
  if ((rc = xsb_command_string(r_th,"test_r_err.")) == XSB_ERROR)
...
void *query_ps_err(void * arg) {
  ...
  rc = xsb_query_string_string(p_th,"p_err(X,Y,Z).",&p_return_string,"|");
  while (rc == XSB_SUCCESS) {
    printf("Pre-err return p %s\n",(p_return_string.string));
    rc = xsb_next_string(p_th, &p_return_string,"|");
  }
