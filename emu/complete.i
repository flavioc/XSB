...
case check_complete: {
...
#ifdef LOCAL_EVAL
  {
	...
	reclaim_stacks(orig_breg);
	{
	  ...
	  CPtr VarsInCall;
#ifdef CHAT
	  CallNumVar = int_val(cell(compl_hreg(cs_ptr)));
	  VarsInCall = compl_hreg(cs_ptr)-CallNumVar;	/* first variable */
#else
...
#endif
	  for (ii=0; ii<CallNumVar; ii++) {
	    CPtr cptr = VarsInCall;
	    pushreg(*cptr);
	    VarsInCall++;
	  } /* for (i=0... */
	}
