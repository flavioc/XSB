...
//  NOTES:
//      PrintErrorMsg() prints out the error message that associates with the statement
//  handler of cursor i.  if i is less than 0, 
//--------------------------------------------------------------------------------------
...
void SetCursorClose(int i)
{
    ...
    CursorTable[i].Sql = 0;
    CursorTable[i].ColNum = CursorTable[i].Status = CursorTable[i].VarListNum = 0;
}
...
//      ODBCConnect()
//  NOTES:
//      This function is called when a user wants to start a db session, assuming that he/she
//   doesn't have one open.  It initializes system resources for the new session, including
//   allocations of various things: environment handler, connection handler, statement
//   handlers and then try to connect to the database indicated by the second parameter
//   prolog passes us using the third one as user id and fourth one as passward.  If any of
//   these allocations or connection fails, function returns a failure code 1.  Otherwise 0.
//---------------------------------------------------------------------------------------------
void ODBCConnect()
{
    int i;
    ...
    // initialize cursor table.  it includes statement handler initialization too.
...
//      ODBCDisconnect()    
//  NOTES:
//      Disconnect us from the server and free all the system resources we allocated for the
//  session - statement handlers, connection handler, environment handler and memory space.
//---------------------------------------------------------------------------------------------
void ODBCDisconnect()
{
    int i;
...
//      FindFreeCursor()
//  NOTES:
//      Find a free statement handler and return its index number into the global cursor
//  table.  It gives priority to a closed cursor with same stantement number over ordinary
//  closed cursors.  If there is no handler left,  function returns -1.
//  possible cursor status values are
...
//      2 - reusing a used cursor w/ the same statement number, no resource needs to be
//          allocated
    ...
    for (i = 0; i < MAXCURSORNUM; i++) {
        if (!CursorTable[i].Status) j = i;                    // a cursor never being used
            ...
            if (CursorTable[i].Status == 1) {                 // a closed cursor
                // and it had the same statement number as this one. so we grab it
...
//     SetBindVarNum() 
//  NOTES:
//      set the number of different bind variables and their total number of occurrances in the
//  sql statement to VarListNum and BListMum respectively and allocate memory for furture use,
//  i.e. for holding the bind variables' types and array of pointers to their value.  note
//  that the memory to store their values is not allocated here since we don't know their type:
//  no information on how much memory is needed.  if we're reusing an old statement handler
//  we don't have to worry about these things.  all we need to do is to make sure that
//  the statement is in deed the same statement w/ the same bind variable number.
//---------------------------------------------------------------------------------------------
void SetBindVarNum()
{
    int i = ptoc_int(2);
...
//      SetVar()   
//  NOTES:
//      set the bind variables' values.  allocate memory if it is needed(status == 3)
//---------------------------------------------------------------------------------------------
void SetVar()
{
    int i = ptoc_int(2);
...
//      SetBind()
//  NOTES:
//      set the bind variables' values for each occurrance of the bind variables in the sql
//  statement.
//---------------------------------------------------------------------------------------------
void SetBind()
{
...
//      Parse()
//  NOTES:
//      parse the sql statement and submit it to DBMS to execute.  if all these succeed, then 
//  prepare for resulting row fetching.  this includes determination of column number in the
//  resulting rowset and the length of each column and memory allocation which is used to store
//  each row.  Note indices -3 and -2 are reserved for transaction control(rollback and commit)
//  and index -1 is for those sql statements that don't need cursor, i.e. they can be executed
//  directly. 
//---------------------------------------------------------------------------------------------
void Parse()
{
    ...
    if (CursorTable[i].Status == 2) {
        ...
        if (SQLPrepare(CursorTable[i].hstmt, CursorTable[i].Sql, SQL_NTS) != SQL_SUCCESS) {
             ctop_int(4,PrintErrorMsg(i));
             SetCursorClose(i);
             return;
        }
        ...
        for (j = 0; j < CursorTable[i].BListNum; j++) {
            if (CursorTable[i].BTypes[j] == 2)
                // we're very sloppy here.  it's ok for us to use the default values
...
//  RETURN VALUE:
//      column length - the size of memory that is needed to store the column value for
...
//      1 - something goes wrong, we can't retrieve column information, memory allocation fails
//          (if this happens program stops).
    ...
    if (CursorTable[i].Status != 2) {
        ...
        for (j = 0; j < CursorTable[i].ColNum; j++) {
            ...
            if (!((CursorTable[i]).ColLen[j] =
                ...
                CursorTable[i].ColNum = j;  // let SetCursorClose function correctly free all
                                            // the memory allocated for Data storage:
                                            // CursorTable[i].Data[j]'s
                SetCursorClose(i);
                return(1);
            }
...
//      FetchNextCol()
//  NOTES:
//      fetch next result rowset.  if we're retrieving user accessable table names, we fetch 
//  until we get next user accessable table name
//---------------------------------------------------------------------------------------------
void FetchNextCol()
{
    int i = ptoc_int(2);
    ...
    if ((rc == SQL_SUCCESS) || (rc == SQL_SUCCESS_WITH_INFO)) 
        ...
        CursorTable[i].Status = 1;    // dome w/ fetching. set cursor status to unused 
        ctop_int(3,1);
    }
    else {
...
//      GetColumn() 
//  NOTES:
//      get the next column.  special care is taken if table information is needed(
//  statement number -2, -1 and 0) since we actually fetch more than what we need.
//  unfortunately it's inevitable.  we discard unwanted columns.  for column info of a
//  table, we need the fourth and fifth columns(statement no 0). for table names in the
//  database and user accessable table names, we only need the third column(statement -1 and
//  -2).
//---------------------------------------------------------------------------------------------
void GetColumn()
{
    ...
    // we set the ColCurNum to some appropriate value to get the columns we need and discard
    // unwanted 
    switch (CursorTable[i].StmtNum) {
            case (0):
    ...
    if (CursorTable[i].OutLen[ColCurNum] == SQL_NULL_DATA) {   // column value is NULL
    ...
    *(CursorTable[i].Data[ColCurNum]+len) = '\0';
    ...
    // pass the result to Prolog
    // if statement is 0, the column type of a table is actually an integer, convert it to
    ...
    // otherwise convert the string to either integer, float or string according to the column
    // type and pass it back to Prolog 
