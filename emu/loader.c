...
byte *loader(char *file, int exp)
{
    ...
    if (magic_num == 0x11121304) {
      ...
      sprintf(message,
	      ...
	      file,
	      "\t supported by CHAT.  Please recompile it.");
      xsb_abort(message);
#else
      if (!warned_old_obj) {
	sprintf(message,
          "File: %s has an old byte code format that may NOT be\n%s%s",
	  file,
	  "\t   supported in the furture.  You are recommended to recompile\n",
	  "\t   all the .O files generated by XSB version 1.x.x.");
	xsb_warn(message);
	warned_old_obj = 1;
      }
#endif
